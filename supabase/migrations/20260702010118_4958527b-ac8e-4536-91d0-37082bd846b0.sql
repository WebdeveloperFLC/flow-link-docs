...
BEGIN
  ...
  LOOP
    ...
    IF v_shared_slug IS NULL OR v_shared_slug = '' THEN CONTINUE; END IF;
    ...
    IF v_shared_id IS NULL THEN
      ...
      VALUES (v_shared_slug, COALESCE(v_shared->>'title', v_shared_slug),
        COALESCE(v_shared->>'article_kind', 'shared'), 'draft',
        jsonb_build_object('tags', jsonb_build_array('shared', 'related-knowledge'),
          'categories', jsonb_build_array('counselling')), v_uid)
      RETURNING id INTO v_shared_id;

      v_body := jsonb_build_object('sections', COALESCE(v_shared->'narrative_sections', '[]'::jsonb))::text;
      INSERT INTO public.kc_article_versions (article_id, version_number, version_label, status, content_format, content_body, created_by)
      VALUES (v_shared_id, 1, '1.0.0', 'draft', 'structured', v_body, v_uid);
      ...
      IF jsonb_array_length(COALESCE(v_shared->'country_codes', '[]'::jsonb)) > 0 THEN
        ...
        SELECT v_shared_id, trim(code) FROM jsonb_array_elements_text(v_shared->'country_codes') AS code
  ...
  INSERT INTO public.kc_articles (slug, title, article_kind, status, metadata, created_by)
  VALUES (v_slug, trim(p_payload->>'title'), COALESCE(p_payload->>'article_kind', 'service'), 'draft',
    jsonb_build_object('tags', COALESCE(p_payload->'tags', '[]'::jsonb),
  ...
  v_body := jsonb_build_object('sections', COALESCE(p_payload->'narrative_sections', '[]'::jsonb))::text;
  INSERT INTO public.kc_article_versions (article_id, version_number, version_label, status, content_format, content_body, created_by)
  VALUES (v_article_id, 1, COALESCE(p_payload->>'version_label', '1.0.0'), 'draft', 'structured', v_body, v_uid)
  ...
  IF jsonb_array_length(COALESCE(p_payload->'country_codes', '[]'::jsonb)) > 0 THEN
    INSERT INTO public.kc_article_countries (article_id, country_code)
    SELECT v_article_id, trim(code) FROM jsonb_array_elements_text(p_payload->'country_codes') AS code
    ON CONFLICT DO NOTHING;
  END IF;
  ...
  IF jsonb_array_length(COALESCE(p_payload->'service_library_ids', '[]'::jsonb)) > 0 THEN
    ...
    SELECT v_article_id, trim(code)::uuid FROM jsonb_array_elements_text(p_payload->'service_library_ids') AS code
    ON CONFLICT DO NOTHING;
  END IF;
  ...
  LOOP
    ...
    VALUES (v_article_id, v_version_id, COALESCE((v_faq->>'sort_order')::int, 0),
      COALESCE(v_faq->>'question', ''), COALESCE(v_faq->>'answer', ''));
  END LOOP;

  FOR v_quiz IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload->'quiz', '[]'::jsonb))
  LOOP
    INSERT INTO public.kc_quiz_questions (article_id, version_id, sort_order, question, options, correct_index, explanation, level)
    VALUES (v_article_id, v_version_id, COALESCE((v_quiz->>'sort_order')::int, 0),
      COALESCE(v_quiz->>'question', ''), COALESCE(v_quiz->'options', '[]'::jsonb),
      COALESCE((v_quiz->>'correct_index')::int, 0), v_quiz->>'explanation', (v_quiz->>'level')::int);
  END LOOP;

  v_sort := 0;
  FOR v_src IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload->'official_sources', '[]'::jsonb))
  LOOP
    INSERT INTO public.kc_official_sources (title, official_url, authority, category, country_code, metadata)
    VALUES (COALESCE(v_src->>'title', ''), COALESCE(v_src->>'official_url', ''),
      COALESCE(v_src->>'authority', ''), COALESCE(v_src->>'category', 'general'),
  ...
  LOOP
    INSERT INTO public.kc_download_assets (article_id, version_id, title, storage_path, download_type, sort_order, metadata)
    VALUES (v_article_id, v_version_id, COALESCE(v_dl->>'title', 'Download'),
      COALESCE(v_dl->>'storage_path', v_slug || '/templates/' || COALESCE(v_dl->>'sort_order', '1') || '.pdf'),
      COALESCE(v_dl->>'download_type', 'other'), COALESCE((v_dl->>'sort_order')::int, 0),
      jsonb_strip_nulls(jsonb_build_object('journey_stage', v_dl->>'journey_stage', 'subtype', v_dl->>'subtype')));
  END LOOP;

  FOR v_rel_slug IN SELECT trim(jsonb_array_elements_text(COALESCE(p_payload->'related_article_slugs', '[]'::jsonb)))
  LOOP
    IF v_rel_slug IS NULL OR v_rel_slug = '' THEN CONTINUE; END IF;
    ...
    IF v_rel_id IS NOT NULL THEN
      ...
      VALUES (v_version_id, v_rel_id, 'related', (SELECT title FROM public.kc_articles WHERE id = v_rel_id))
  ...
  IF p_publish THEN
    ...
    UPDATE public.kc_article_versions SET status = 'published', published_at = now(),
      published_by = v_uid, updated_at = now() WHERE id = v_version_id;
    UPDATE public.kc_articles SET current_version_id = v_version_id, status = 'published',
      updated_at = now() WHERE id = v_article_id;
  END IF;

  RETURN jsonb_build_object('article_id', v_article_id, 'version_id', v_version_id, 'slug', v_slug,
    'counts', jsonb_build_object(
      'narrative_sections', jsonb_array_length(COALESCE(p_payload->'narrative_sections', '[]'::jsonb)),
...
GRANT EXECUTE ON FUNCTION public.kc_import_guide(jsonb, boolean, boolean) TO authenticated;
COMMENT ON FUNCTION public.kc_import_guide(jsonb, boolean, boolean) IS
  'Import a Knowledge Centre guide JSON payload into kc_* tables (sections, satellites).';