-- 1) Idempotent Phase 2B schema catch-up
CREATE TABLE IF NOT EXISTS public.upi_commission_aggregator_invoices (
  ...
  status text NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','submitted','approved','partially_paid','paid','disputed','cancelled')),
...
CREATE TABLE IF NOT EXISTS public.upi_commission_aggregator_invoice_lines (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  aggregator_invoice_id uuid NOT NULL REFERENCES public.upi_commission_aggregator_invoices(id) ON DELETE CASCADE,
...
BEGIN
  IF to_regclass('public.upi_aggregators') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'upi_commission_invoices_aggregator_id_fkey') THEN
    ALTER TABLE public.upi_commission_invoices
  ...
  IF to_regclass('public.upi_commission_aggregator_invoices') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'upi_commission_invoices_aggregator_invoice_id_fkey') THEN
    ALTER TABLE public.upi_commission_invoices
      ADD CONSTRAINT upi_commission_invoices_aggregator_invoice_id_fkey
      FOREIGN KEY (aggregator_invoice_id) REFERENCES public.upi_commission_aggregator_invoices(id) ON DELETE SET NULL;
  END IF;
END $$;
...
DO $$
BEGIN
  IF to_regclass('public.upi_commission_aggregator_invoice_lines') IS NULL THEN RETURN; END IF;
  UPDATE public.upi_commission_invoices inv
  SET aggregator_invoice_id = ail.aggregator_invoice_id,
...
CREATE OR REPLACE FUNCTION public.fn_resolve_aggregator_invoice_for_institution_invoice(p_invoice_id uuid)
...
BEGIN
  ...
  IF to_regclass('public.upi_commission_aggregator_invoice_lines') IS NOT NULL THEN
    SELECT ail.aggregator_invoice_id INTO v_ai
    FROM public.upi_commission_aggregator_invoice_lines ail
    WHERE ail.institution_invoice_id = p_invoice_id
    ORDER BY ail.sort_order, ail.id LIMIT 1;
    IF v_ai IS NOT NULL THEN RETURN v_ai; END IF;
  END IF;
  SELECT inv.aggregator_invoice_id INTO v_ai
  FROM public.upi_commission_invoices inv WHERE inv.id = p_invoice_id;
  RETURN v_ai;
END; $$;

-- 3) Aggregator invoice maintenance
CREATE OR REPLACE FUNCTION public.fn_add_invoices_to_aggregator_invoice(p_aggregator_invoice_id uuid, p_institution_invoice_ids uuid[])
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_inv uuid; v_inst uuid; v_amount numeric; v_agg uuid; v_status text;
BEGIN
  PERFORM public.fn_assert_commission_aggregator_actor();
  SELECT aggregator_id, status INTO v_agg, v_status FROM public.upi_commission_aggregator_invoices WHERE id = p_aggregator_invoice_id;
  IF v_status <> 'draft' THEN RAISE EXCEPTION 'only draft aggregator invoices can be edited'; END IF;

  FOREACH v_inv IN ARRAY p_institution_invoice_ids LOOP
    IF EXISTS (
      SELECT 1 FROM public.upi_commission_aggregator_invoice_lines ail
      JOIN public.upi_commission_aggregator_invoices ai ON ai.id = ail.aggregator_invoice_id
      WHERE ail.institution_invoice_id = v_inv AND ai.status NOT IN ('cancelled') AND ai.id <> p_aggregator_invoice_id
    ) THEN
      RAISE EXCEPTION 'institution invoice % already linked to another aggregator invoice', v_inv;
    END IF;

    SELECT institution_id, total_amount INTO v_inst, v_amount FROM public.upi_commission_invoices WHERE id = v_inv;

    INSERT INTO public.upi_commission_aggregator_invoice_lines (aggregator_invoice_id, institution_id, institution_invoice_id, line_amount, amount_outstanding)
    VALUES (p_aggregator_invoice_id, v_inst, v_inv, v_amount, v_amount)
    ON CONFLICT (aggregator_invoice_id, institution_invoice_id) DO UPDATE SET line_amount = EXCLUDED.line_amount;

    UPDATE public.upi_commission_invoices SET aggregator_id = v_agg, aggregator_invoice_id = p_aggregator_invoice_id WHERE id = v_inv;
  END LOOP;
...
DECLARE
  ...
  sa record; v_snap uuid; v_inv record; v_st record; v_ai uuid;
BEGIN
  ...
  ELSE RAISE EXCEPTION 'receipt must be draft or ready to post';
  END IF;

  FOR sa IN SELECT * FROM public.upi_commission_receipt_student_allocations WHERE receipt_id = p_receipt_id LOOP
  ...
  UPDATE public.upi_commission_receipts SET status='posted', posted_at=now(), posted_by=auth.uid(), posting_date=COALESCE(posting_date, receipt_date) WHERE id = p_receipt_id;

  FOR v_inv IN SELECT DISTINCT ia.invoice_id AS invoice_id FROM public.upi_commission_receipt_invoice_allocations ia WHERE ia.receipt_id = p_receipt_id LOOP
  ...
  FOR v_st IN SELECT DISTINCT sa.student_commission_id AS student_commission_id FROM public.upi_commission_receipt_student_allocations sa WHERE sa.receipt_id = p_receipt_id LOOP
  ...
  IF r.remittance_batch_id IS NOT NULL THEN PERFORM public.fn_refresh_remittance_batch_totals(r.remittance_batch_id); END IF;
END; $$;

CREATE OR REPLACE FUNCTION public.fn_void_commission_receipt(p_receipt_id uuid, p_reason text DEFAULT NULL)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE r public.upi_commission_receipts%ROWTYPE; v_inv uuid; v_st uuid; v_ai uuid;
BEGIN
  PERFORM public.fn_assert_commission_receipt_actor();
  SELECT * INTO r FROM public.upi_commission_receipts WHERE id = p_receipt_id FOR UPDATE;

  IF r.status = 'draft' THEN
    UPDATE public.upi_commission_receipts SET status='voided', voided_at=now(), voided_by=auth.uid(), void_reason=p_reason WHERE id = p_receipt_id;
    RETURN;
  END IF;

  IF r.status <> 'posted' THEN RAISE EXCEPTION 'only posted or draft receipts can be voided'; END IF;

  UPDATE public.upi_commission_receipts SET status='voided', voided_at=now(), voided_by=auth.uid(), void_reason=p_reason WHERE id = p_receipt_id;

  FOR v_inv IN SELECT DISTINCT invoice_id FROM public.upi_commission_receipt_invoice_allocations WHERE receipt_id = p_receipt_id LOOP
  ...
  FOR v_st IN SELECT DISTINCT student_commission_id FROM public.upi_commission_receipt_student_allocations WHERE receipt_id = p_receipt_id LOOP
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='upi_commission_invoices' AND column_name='aggregator_invoice_id') THEN
