-- Accounting Hardening Phase A1
-- Lifecycle statuses, dependency RPC, audit action extensions.
-- No refund engine, no transfer posting, no immutability changes.
...
-- ---------------------------------------------------------------------------
-- 2. Extend service audit log actions (transfer skeleton)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Helper: invoice line matches service keys
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_invoices_count > 0 THEN
    ...
    WHERE a.invoice_id = ANY (v_invoice_ids)
      AND (
        a.service_id::text = ANY (v_keys)
        OR a.amount_allocated > 0
      );

    SELECT COUNT(*)::int INTO v_receipts_count
    FROM public.client_invoice_receipts r
    WHERE r.invoice_id = ANY (v_invoice_ids)
    ...
    WHERE b.invoice_id = ANY (v_invoice_ids)
  ...
  -- Payments without matching invoice lines but with service allocation
  ...
  v_has_financial := v_has_financial
    ...
    OR v_discounts_count > 0
    OR v_wallet_count > 0
    OR v_journals_count > 0
    OR (v_trust_count > 0 AND v_invoices_count > 0);
