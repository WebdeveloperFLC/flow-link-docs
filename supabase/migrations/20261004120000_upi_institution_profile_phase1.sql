-- M1 — Institution Master profile enhancement (Phase 1)
-- Adds governance profile fields, source tracking, human verification, completeness score.
-- Does NOT modify Mark Final, institution_fee_schedule accuracy rules, or contacts (M2).
--
-- PRESERVED (ws-2 / locked): APPLICATION fee_accuracy = EXACT; TUITION/DEPOSIT = APPROXIMATE.
-- See institution_fee_schedule_application_exact_chk in 20261001120000_institution_fee_schedule_ws2.sql

-- ---------------------------------------------------------------------------
-- Profile + governance columns
-- ---------------------------------------------------------------------------
...
-- Drop constraints if re-run locally, then re-add
ALTER TABLE public.upi_institutions
...
-- ---------------------------------------------------------------------------
-- Normalize legacy institution_type values
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Backfill institution_status from legacy is_active (M5 will remediate shells)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Completeness score (Institution Completeness Score)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Compliance (15) — Canada-weighted; non-Canada gets description credit instead
  IF v_is_canada THEN
...
-- Recompute scores for existing rows
...
-- Fee accuracy rule preserved — document only (no DDL change to institution_fee_schedule)
