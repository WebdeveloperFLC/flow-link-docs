-- HR Payroll — Professional Tax: ₹200 default for all India employees (amount via policy)
-- PT amount is read from policies.professional_tax.config.default_amount (changeable in /hr/config)

-- ---- Schema + view (migration 17 gap on some staging DBs) ----
...
-- India payroll: PT on for all staff (Canada/CAD excluded)
UPDATE employees
...
-- Apply PT on latest cycle lines (including Paid/Locked — data correction)
DO $$
DECLARE
