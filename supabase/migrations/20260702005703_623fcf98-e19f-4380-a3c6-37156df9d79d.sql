...
COMMENT ON COLUMN public.upi_institutions.approximate_tuition_range IS
  'Informational counselor guidance only (e.g. Approx. CAD 15,000-22,000/year). Does not replace institution_fee_schedule TUITION rows or billing.';

COMMENT ON COLUMN public.upi_institutions.approximate_deposit_range IS
  'Informational counselor guidance only (e.g. Approx. CAD 2,000-5,000). Does not replace institution_fee_schedule DEPOSIT rows or billing.';
