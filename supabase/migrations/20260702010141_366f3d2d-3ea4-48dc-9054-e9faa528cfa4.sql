...
DECLARE
  ...
  v_version_id uuid;
BEGIN
  SELECT id INTO v_article_id FROM public.kc_articles WHERE slug = 'canada-student-visa-outside-canada';
  IF v_article_id IS NULL THEN
    RAISE NOTICE 'kc_canada_guide_service_linkage: no article — import required';
    RETURN;
  END IF;
  ...
  WHERE sl.id = v_canonical
     OR (sl.service_category = 'visa_immigration'
         AND sl.service = 'Canada'
         AND sl.is_active = true
         AND (COALESCE(sl.academy_metadata->>'displayName', '') ILIKE '%Student Visa%Outside Canada%'
              OR sl.sub_service ILIKE '%Student Visa%Outside%'
              OR sl.sub_service ILIKE '%Study Permit%Undergraduate%'))
  ON CONFLICT DO NOTHING;

  SELECT id INTO v_version_id FROM public.kc_article_versions
  WHERE article_id = v_article_id AND status = 'published'
  ORDER BY version_number DESC LIMIT 1;

  IF v_version_id IS NOT NULL THEN
    UPDATE public.kc_articles
    SET current_version_id = COALESCE(current_version_id, v_version_id),
