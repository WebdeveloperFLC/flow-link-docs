-- CF ↔ UPI institution linkage audit (Phase 1)

DO $$ BEGIN
  CREATE TYPE public.cf_upi_linkage_match_method AS ENUM (
    'already_linked','exact','normalized','alias','manual','unmatched'
  );
...
DO $$ BEGIN
  CREATE TYPE public.cf_upi_linkage_candidate_status AS ENUM (
    'pending_review','approved','applied','rejected','superseded'
  );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_cf_upi_linkage_runs_started ON public.cf_upi_linkage_runs (started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_cf_upi_name_aliases_active ON public.cf_upi_name_aliases (is_active) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_cf_upi_linkage_candidates_run ON public.cf_upi_linkage_candidates (run_id);
CREATE INDEX IF NOT EXISTS idx_cf_upi_linkage_candidates_status ON public.cf_upi_linkage_candidates (status);
CREATE INDEX IF NOT EXISTS idx_cf_upi_linkage_candidates_cf ON public.cf_upi_linkage_candidates (cf_university_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_cf_upi_linkage_candidates_open_unique
  ON public.cf_upi_linkage_candidates (cf_university_id) WHERE status IN ('pending_review', 'approved');

DROP TRIGGER IF EXISTS cf_upi_name_aliases_touch ON public.cf_upi_name_aliases;
CREATE TRIGGER cf_upi_name_aliases_touch BEFORE UPDATE ON public.cf_upi_name_aliases
...
CREATE POLICY cf_upi_linkage_runs_select ON public.cf_upi_linkage_runs FOR SELECT TO authenticated
...
CREATE POLICY cf_upi_linkage_candidates_select ON public.cf_upi_linkage_candidates FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::public.app_role)
    OR public.user_has_module(auth.uid(), 'institutions', 'view')
    OR public.user_has_module(auth.uid(), 'institutions', 'edit'));

DROP POLICY IF EXISTS cf_upi_name_aliases_select ON public.cf_upi_name_aliases;
CREATE POLICY cf_upi_name_aliases_select ON public.cf_upi_name_aliases FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::public.app_role)
    OR public.user_has_module(auth.uid(), 'institutions', 'view')
    OR public.user_has_module(auth.uid(), 'institutions', 'edit'));

GRANT SELECT, INSERT, UPDATE, DELETE ON public.cf_upi_linkage_runs TO authenticated;
GRANT ALL ON public.cf_upi_linkage_runs TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.cf_upi_linkage_candidates TO authenticated;
GRANT ALL ON public.cf_upi_linkage_candidates TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.cf_upi_name_aliases TO authenticated;
GRANT ALL ON public.cf_upi_name_aliases TO service_role;

COMMENT ON TABLE public.cf_upi_linkage_runs IS 'Audit log for CF↔UPI linkage dry-run and apply batches.';
COMMENT ON TABLE public.cf_upi_linkage_candidates IS 'Review queue: proposed cf_universities → upi_institutions mappings.';
COMMENT ON TABLE public.cf_upi_name_aliases IS 'Configurable CF name → UPI institution alias rules.';