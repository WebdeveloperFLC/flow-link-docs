...
DECLARE
  ...
  v_is_canada boolean;
BEGIN
  SELECT * INTO v_row FROM public.upi_institutions WHERE id = _institution_id;
  ...
  IF v_row.name IS NULL OR trim(v_row.name) = '' THEN
    v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_name', 'message', 'Institution name is required'));
  END IF;
  IF v_row.institution_type IS NULL OR trim(v_row.institution_type) = '' THEN
    v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_institution_type', 'message', 'Institution type is required'));
  END IF;
  IF (v_row.country_id IS NULL) AND (v_row.country_name IS NULL OR trim(v_row.country_name) = '') THEN
    v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_country', 'message', 'Country is required'));
  END IF;
  IF v_row.website_url IS NULL OR trim(v_row.website_url) = '' THEN
    v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_website', 'message', 'Official website URL is required'));
  END IF;
  IF v_row.application_portal_url IS NULL OR trim(v_row.application_portal_url) = '' THEN
    v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_application_portal', 'message', 'Application portal URL is required'));
  END IF;
  IF v_is_canada THEN
    IF v_row.dli_number IS NULL OR trim(v_row.dli_number) = '' THEN
      v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_dli', 'message', 'DLI number is required for Canadian institutions'));
    END IF;
    IF v_row.pgwp_eligible IS NULL THEN
      v_errors := v_errors || jsonb_build_array(jsonb_build_object('code', 'missing_pgwp', 'message', 'PGWP eligibility must be set (Yes or No) for Canadian institutions'));
    END IF;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.institution_fee_schedule f
    WHERE f.upi_institution_id = _institution_id AND f.fee_type = 'APPLICATION' AND f.status = 'ACTIVE'
      AND f.program_id IS NULL AND f.partnership_route_id IS NULL
  ) THEN
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object('code', 'missing_application_fee', 'message', 'No active institution-default application fee on Fee Schedule'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.institution_fee_schedule f
    WHERE f.upi_institution_id = _institution_id AND f.fee_type = 'TUITION' AND f.status = 'ACTIVE'
      AND f.fee_accuracy = 'APPROXIMATE' AND f.program_id IS NULL AND f.partnership_route_id IS NULL
  ) THEN
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object('code', 'missing_tuition_fee', 'message', 'No active approximate tuition fee on Fee Schedule'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.institution_fee_schedule f
    WHERE f.upi_institution_id = _institution_id AND f.fee_type = 'DEPOSIT' AND f.status = 'ACTIVE'
      AND f.fee_accuracy = 'APPROXIMATE' AND f.program_id IS NULL AND f.partnership_route_id IS NULL
  ) THEN
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object('code', 'missing_deposit_fee', 'message', 'No active approximate deposit fee on Fee Schedule'));
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.upi_institution_contacts c
    WHERE c.institution_id = _institution_id AND c.is_active = true
      AND c.email IS NOT NULL AND trim(c.email) <> ''
  ) THEN
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object('code', 'missing_contacts', 'message', 'No active institution contact with email (recommended)'));
  END IF;
  IF v_row.last_loa_verified_at IS NULL THEN
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object('code', 'missing_loa_verified', 'message', 'Last LOA verified date is empty (recommended)'));
  END IF;
...
BEGIN
  ...
  SELECT * INTO v_row FROM public.upi_institutions WHERE id = p_institution_id FOR UPDATE;
  ...
  IF p_status = 'Active' AND v_row.institution_status IS DISTINCT FROM 'Active' THEN
    ...
    IF NOT p_force_warnings AND jsonb_array_length(v_warnings) > 0 THEN
      RETURN jsonb_build_object('ok', false, 'needs_confirmation', true, 'errors', '[]'::jsonb, 'warnings', v_warnings);
    END IF;
  END IF;
  ...
  SET institution_status = p_status, updated_at = now()
  WHERE id = p_institution_id;

  RETURN jsonb_build_object('ok', true, 'institution_status', p_status,
...
BEGIN
  ...
  IF TG_OP = 'UPDATE'
    ...
    v_errors := coalesce(v_validation->'errors', '[]'::jsonb);
    IF jsonb_array_length(v_errors) > 0 THEN
      RAISE EXCEPTION 'institution_activation_blocked: %', v_errors::text USING ERRCODE = 'check_violation';
    END IF;
  END IF;
  RETURN NEW;
END;
