-- Fix Register as Client FK failure: leads_converted_to_client_id_fkey requires clients.id
-- to exist before leads.converted_to_client_id is set. Split BEFORE (client fields) vs AFTER (lead link).

-- ---------------------------------------------------------------------------
-- 1) BEFORE INSERT/UPDATE — source_lead_number only (no lead UPDATE)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_client_source_lead_before()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ---------------------------------------------------------------------------
-- 2) AFTER INSERT/UPDATE — mark lead converted (client row exists; FK safe)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_mark_lead_converted_after_client()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- One-time repair: clients created while BEFORE trigger violated FK may lack lead back-link.
UPDATE public.leads l
 ...
 WHERE c.source_lead_id = l.id
   AND l.converted_to_client_id IS DISTINCT FROM c.id;

DROP TRIGGER IF EXISTS trg_client_source_lead_before ON public.clients;
CREATE TRIGGER trg_client_source_lead_before
  BEFORE INSERT OR UPDATE OF source_lead_id ON public.clients
  FOR EACH ROW EXECUTE FUNCTION public.fn_client_source_lead_before();

DROP TRIGGER IF EXISTS trg_mark_lead_converted_after_client ON public.clients;
CREATE TRIGGER trg_mark_lead_converted_after_client
  AFTER INSERT OR UPDATE OF source_lead_id ON public.clients
  FOR EACH ROW EXECUTE FUNCTION public.fn_mark_lead_converted_after_client();

-- ---------------------------------------------------------------------------
-- 3) convert_lead_to_client — backfill lead when client already exists for source_lead_id
-- ---------------------------------------------------------------------------
