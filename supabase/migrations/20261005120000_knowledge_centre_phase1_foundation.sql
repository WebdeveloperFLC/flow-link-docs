-- Knowledge Centre Phase 1 — schema, RLS, storage, RPCs
-- Contract: Functional Specification v1.0 + Architecture Validation
-- SSOT: counselling knowledge only; FK to countries.code + service_library.id
...
COMMENT ON FUNCTION public.can_view_knowledge_centre(uuid) IS
  'Staff read access for Knowledge Centre (module perm or legacy service-library staff).';
COMMENT ON FUNCTION public.can_manage_knowledge_centre(uuid) IS
  'KC authoring, publish, official sources edit.';
...
-- ---------------------------------------------------------------------------
-- 2. Official sources registry (SSOT for URLs)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_kc_official_sources_review
  ON public.kc_official_sources (status, last_verified_at);
CREATE INDEX IF NOT EXISTS idx_kc_official_sources_country
  ON public.kc_official_sources (country_code);
...
CREATE INDEX IF NOT EXISTS idx_kc_articles_kind_status
  ON public.kc_articles (article_kind, status);
CREATE INDEX IF NOT EXISTS idx_kc_articles_slug
  ON public.kc_articles (slug);
...
CREATE INDEX IF NOT EXISTS idx_kc_versions_article
  ON public.kc_article_versions (article_id, version_number DESC);

ALTER TABLE public.kc_articles
  DROP CONSTRAINT IF EXISTS kc_articles_current_version_fkey;
ALTER TABLE public.kc_articles
  ADD CONSTRAINT kc_articles_current_version_fkey
...
CREATE INDEX IF NOT EXISTS idx_kc_article_countries_code
  ON public.kc_article_countries (country_code);
...
CREATE INDEX IF NOT EXISTS idx_kc_article_services_lib
  ON public.kc_article_services (service_library_id);

-- ---------------------------------------------------------------------------
-- 5. Content satellites
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.kc_download_assets (
  ...
  download_type text NOT NULL DEFAULT 'other'
    CHECK (download_type IN (
      'counsellor_guide', 'meeting_checklist', 'budget_planner',
      'arrival_checklist', 'settlement_checklist', 'other'
    )),
  title text NOT NULL,
...
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    'kc_official_sources', 'kc_articles', 'kc_article_versions'
  ])
  LOOP
...
CREATE POLICY kc_sources_select ON public.kc_official_sources
...
CREATE POLICY kc_sources_manage ON public.kc_official_sources
  FOR ALL TO authenticated
  USING (public.can_manage_knowledge_centre(auth.uid()))
  WITH CHECK (public.can_manage_knowledge_centre(auth.uid()));

-- Articles: published visible to viewers; drafts to managers
DROP POLICY IF EXISTS kc_articles_select ON public.kc_articles;
CREATE POLICY kc_articles_select ON public.kc_articles
  FOR SELECT TO authenticated
  USING (
    public.can_manage_knowledge_centre(auth.uid())
    OR (
      public.can_view_knowledge_centre(auth.uid())
      AND status IN ('published', 'archived')
    )
  );

DROP POLICY IF EXISTS kc_articles_manage ON public.kc_articles;
CREATE POLICY kc_articles_manage ON public.kc_articles
  FOR ALL TO authenticated
  USING (public.can_manage_knowledge_centre(auth.uid()))
  WITH CHECK (public.can_manage_knowledge_centre(auth.uid()));

-- Versions: published versions readable; draft only managers
DROP POLICY IF EXISTS kc_versions_select ON public.kc_article_versions;
CREATE POLICY kc_versions_select ON public.kc_article_versions
  FOR SELECT TO authenticated
  USING (
    public.can_manage_knowledge_centre(auth.uid())
    OR (
...
CREATE POLICY kc_versions_manage ON public.kc_article_versions
  FOR ALL TO authenticated
  USING (public.can_manage_knowledge_centre(auth.uid()))
  WITH CHECK (public.can_manage_knowledge_centre(auth.uid()));

-- Junction + satellites: follow article access
DO $$
DECLARE
  tbl text;
  short text;
BEGIN
  FOR tbl IN SELECT unnest(ARRAY[
    'kc_article_countries', 'kc_article_services', 'kc_faq_items',
  ...
  LOOP
    ...
    EXECUTE format(
      'CREATE POLICY kc_%s_select ON public.%I FOR SELECT TO authenticated
       USING (public.can_view_knowledge_centre(auth.uid()) OR public.can_manage_knowledge_centre(auth.uid()))',
      short, tbl
    );
    EXECUTE format('DROP POLICY IF EXISTS kc_%s_manage ON public.%I', short, tbl);
    EXECUTE format(
      'CREATE POLICY kc_%s_manage ON public.%I FOR ALL TO authenticated
       USING (public.can_manage_knowledge_centre(auth.uid()))
       WITH CHECK (public.can_manage_knowledge_centre(auth.uid()))',
      short, tbl
    );
...
VALUES (
  ...
  false,
  52428800,
  ARRAY['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel', 'text/csv', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
)
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS kc_downloads_read ON storage.objects;
CREATE POLICY kc_downloads_read ON storage.objects
  FOR SELECT TO authenticated
  USING (
    bucket_id = 'kc-downloads'
    AND public.can_view_knowledge_centre(auth.uid())
  );

DROP POLICY IF EXISTS kc_downloads_insert ON storage.objects;
CREATE POLICY kc_downloads_insert ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id = 'kc-downloads'
    AND public.can_manage_knowledge_centre(auth.uid())
  );

DROP POLICY IF EXISTS kc_downloads_update ON storage.objects;
CREATE POLICY kc_downloads_update ON storage.objects
...
CREATE POLICY kc_downloads_delete ON storage.objects
...
-- ---------------------------------------------------------------------------
-- 10. RPC: resolve live article
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.kc_resolve_live_article(
...
BEGIN
  IF NOT public.can_view_knowledge_centre(auth.uid()) THEN
...
-- ---------------------------------------------------------------------------
-- 11. RPC: search articles
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.kc_search_articles(
  p_query text DEFAULT NULL,
  p_country_code text DEFAULT NULL,
  p_service_library_id uuid DEFAULT NULL,
  p_article_kind text DEFAULT NULL,
...
BEGIN
  ...
  SELECT
    ...
    a.status,
    v.version_label,
    LEFT(COALESCE(v.content_body, ''), 200) AS snippet,
    COALESCE(a.metadata->'tags', '[]'::jsonb) AS tags,
    COALESCE(a.metadata->'categories', '[]'::jsonb) AS categories
  ...
  WHERE
    ...
    AND (
      ...
      OR EXISTS (
        SELECT 1
        FROM jsonb_array_elements_text(COALESCE(a.metadata->'tags', '[]'::jsonb)) tag
