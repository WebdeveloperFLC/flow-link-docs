-- Lead autosave bypasses broken/conflicting RLS on direct INSERT.
-- SECURITY DEFINER RPCs (same pattern as create_lead_from_family_member).

CREATE OR REPLACE FUNCTION public._lead_json_text_array(_data jsonb, _key text)
RETURNS text[]
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT COALESCE(
    ARRAY(SELECT jsonb_array_elements_text(COALESCE(_data -> _key, '[]'::jsonb))),
    '{}'::text[]
  );
$$;

CREATE OR REPLACE FUNCTION public._assert_can_manage_leads(_uid uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Client-portal-only accounts cannot create CRM leads.
  IF public.has_role(_uid, 'client'::public.app_role)
     AND NOT EXISTS (
       SELECT 1
...
CREATE OR REPLACE FUNCTION public._assert_can_edit_lead(_uid uuid, _lead_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  PERFORM public._assert_can_manage_leads(_uid);
  ...
  IF EXISTS (
    SELECT 1
     ...
     WHERE l.id = _lead_id
       AND (
         l.created_by = _uid
         OR l.assigned_counselor_id = _uid
         OR public.is_client_staff_editor(_uid)
         OR public.is_client_operational_staff(_uid)
...
GRANT EXECUTE ON FUNCTION public.create_lead_draft(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.patch_lead_draft(uuid, jsonb) TO authenticated;
