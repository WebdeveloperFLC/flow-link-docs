-- French + German language tests (CEFR A1-C2) on leads and clients; updates lead RPCs.
...
COMMENT ON COLUMN public.leads.language_tests IS 'French/german blocks {french:{status,cefr_level,exam_type,...}, german:{...}}';

-- Ensure background columns exist (no-op if migration 40 already applied).
ALTER TABLE public.leads
...
BEGIN
  ...
  UPDATE public.leads l
     SET
       ...
       END,
       language_tests = CASE
         WHEN _data ? 'language_tests' THEN COALESCE(_data -> 'language_tests', '{}'::jsonb)
         ELSE l.language_tests
       END,
...
-- Move legacy DELF/TestDaF rows from other_tests into language_tests.
DO $$
DECLARE
  r record;
  elem jsonb;
  ...
  typ text;
BEGIN
  FOR r IN SELECT id, other_tests, language_tests FROM public.leads LOOP
    kept := '[]'::jsonb;
    lt := COALESCE(r.language_tests, '{}'::jsonb);
    FOR elem IN SELECT value FROM jsonb_array_elements(COALESCE(r.other_tests, '[]'::jsonb)) LOOP
      typ := upper(coalesce(elem->>'type', ''));
      IF typ IN ('DELF', 'DALF', 'TCF', 'TEF') AND NOT (lt ? 'french') THEN
        lt := lt || jsonb_build_object('french', jsonb_build_object(
          'status', 'taken',
          'exam_type', elem->>'type',
          'overall_score', elem->>'score',
          'test_date', elem->>'date',
          'sections', COALESCE(elem->'sections', '{}'::jsonb)
        ));
      ELSIF typ IN ('TESTDAF', 'GOETHE', 'DSH', 'TELC') AND NOT (lt ? 'german') THEN
        lt := lt || jsonb_build_object('german', jsonb_build_object(
          'status', 'taken',
          'exam_type', elem->>'type',
          'overall_score', elem->>'score',
          'test_date', elem->>'date',
      ...
      END IF;
    END LOOP;
    IF lt IS DISTINCT FROM COALESCE(r.language_tests, '{}'::jsonb)
       OR kept IS DISTINCT FROM COALESCE(r.other_tests, '[]'::jsonb) THEN
      UPDATE public.leads SET language_tests = lt, other_tests = kept WHERE id = r.id;
    END IF;
  END LOOP;

  FOR r IN SELECT id, other_tests, language_tests FROM public.clients LOOP
    kept := '[]'::jsonb;
    lt := COALESCE(r.language_tests, '{}'::jsonb);
    FOR elem IN SELECT value FROM jsonb_array_elements(COALESCE(r.other_tests, '[]'::jsonb)) LOOP
      typ := upper(coalesce(elem->>'type', ''));
    ...
    IF lt IS DISTINCT FROM COALESCE(r.language_tests, '{}'::jsonb)
       OR kept IS DISTINCT FROM COALESCE(r.other_tests, '[]'::jsonb) THEN
      UPDATE public.clients SET language_tests = lt, other_tests = kept WHERE id = r.id;
    END IF;
  END LOOP;
END $$;
...
CREATE TRIGGER clients_sync_client_profile
  AFTER INSERT OR UPDATE OF
    ...
    other_tests, education_history, work_experience, language_tests
  ON public.clients
  FOR EACH ROW
