-- Phase D UAT: convert_lead_to_client wrote integer year into clients.year_of_passing (date column).
...
BEGIN
  ...
  v_year_raw := NULLIF(btrim(v_e0 ->> 'year'), '');
  v_year_date := CASE
    WHEN v_year_raw IS NULL THEN NULL
