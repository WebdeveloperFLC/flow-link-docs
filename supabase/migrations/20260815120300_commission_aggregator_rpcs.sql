...
BEGIN
  ...
  FOR v_stu_alloc IN SELECT * FROM public.upi_commission_receipt_student_allocations WHERE receipt_id = p_receipt_id
  LOOP
    IF v_stu_alloc.snapshot_id IS NOT NULL THEN
      SELECT commission_snapshot_id INTO v_snap FROM public.upi_commission_students WHERE id = v_stu_alloc.student_commission_id;
  ...
  FOR v_st IN
    ...
    FROM public.upi_commission_receipt_student_allocations rsa WHERE rsa.receipt_id = p_receipt_id
  LOOP
