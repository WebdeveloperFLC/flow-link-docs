...
-- Recovery re-apply of 20260722120000 with service_catalogue guard
-- =====================================================================
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.accounting_collection_categories TO authenticated;
GRANT ALL ON public.accounting_collection_categories TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.accounting_collection_category_coa TO authenticated;
GRANT ALL ON public.accounting_collection_category_coa TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.accounting_collection_category_vendors TO authenticated;
GRANT ALL ON public.accounting_collection_category_vendors TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.accounting_category_audit_log TO authenticated;
GRANT ALL ON public.accounting_category_audit_log TO service_role;
...
-- Legacy compatibility guard: service_catalogue was retired in 20260610190000.
-- Apply the ALTER only if the retired table still exists.
DO $legacy_svc_cat$
BEGIN
...
DROP TRIGGER IF EXISTS trg_collection_categories_updated_at ON public.accounting_collection_categories;
CREATE TRIGGER trg_collection_categories_updated_at
  BEFORE UPDATE ON public.accounting_collection_categories
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

DROP TRIGGER IF EXISTS trg_collection_category_coa_updated_at ON public.accounting_collection_category_coa;
