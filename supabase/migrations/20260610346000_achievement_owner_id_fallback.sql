-- CRM stores primary counsellor on clients.owner_id; wallet engine used assigned_counselor_id only.
-- Fall back to owner_id so achievement / scores work without manual column sync.
...
-- Re-apply target_metric fix with owner_id fallback in pay_rev (same as 20260610345000)
...
COMMENT ON FUNCTION public.fn_counselor_period_achievement(text) IS
  'Per-counsellor achievement %. Uses COALESCE(assigned_counselor_id, owner_id) on clients for revenue attribution.';
