...
create policy slip_select on salary_slips for select using (
  is_hr(org_id) or employee_id = current_employee_id(org_id));
create policy slip_write on salary_slips for all using (has_perm(org_id,'export') or has_perm(org_id,'approve'))
  with check (has_perm(org_id,'export') or has_perm(org_id,'approve'));
