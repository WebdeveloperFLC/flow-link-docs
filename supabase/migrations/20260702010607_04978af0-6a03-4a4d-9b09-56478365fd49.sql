...
VALUES
  ('approval_required', 'false'::jsonb, 'When true (Phase 4+), financial RPCs route through maker-checker. Phase 3 ships false.'),
  ('financial_events_enabled', 'false'::jsonb, 'When true (F3.1+), money-moving RPCs emit rows to upi_commission_financial_events.'),
  ('financial_events_table', '"upi_commission_financial_events"'::jsonb, 'Canonical financial-events table name for Commission → Finance boundary.'),
  ('audit_log_enabled', 'false'::jsonb, 'When true (F3.3+), mutating financial RPCs write upi_commission_audit_log rows.')
