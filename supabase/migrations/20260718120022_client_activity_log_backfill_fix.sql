-- Idempotent bootstrap + backfill for client_activity_log.
-- Run this alone in SQL editor if 20260718120021 failed or was skipped.
-- Safe to re-run: creates missing objects and skips already-backfilled rows.
...
-- Unique per client + source (multiple clients may share source_lead_id)
...
WHERE NOT EXISTS (
  ...
  WHERE cal.source_table = 'client_timeline' AND cal.source_id = ct.id AND cal.client_id = ct.client_id
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.client_activity_log cal
  WHERE cal.source_table = 'client_stage_completion_log' AND cal.source_id = log.id AND cal.client_id = log.client_id
);

-- Backfill: stage history
INSERT INTO public.client_activity_log (
  client_id, actor_id, actor_role, action, summary, previous_value, new_value, metadata,
  source_table, source_id, created_at
)
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.client_activity_log cal
  WHERE cal.source_table = 'client_stage_history' AND cal.source_id = h.id AND cal.client_id = h.client_id
);

-- Backfill: activity_logs (client entity)
INSERT INTO public.client_activity_log (
...
WHERE al.entity_type = 'client'
  ...
  AND NOT EXISTS (
    SELECT 1 FROM public.client_activity_log cal
    WHERE cal.source_table = 'activity_logs' AND cal.source_id = al.id AND cal.client_id = al.entity_id
  );

-- Backfill: activity_logs (document entity)
INSERT INTO public.client_activity_log (
  client_id, actor_id, actor_role, action, summary, metadata,
  source_table, source_id, created_at
)
...
WHERE al.entity_type = 'document'
  AND NOT EXISTS (
    SELECT 1 FROM public.client_activity_log cal
    WHERE cal.source_table = 'activity_logs' AND cal.source_id = al.id AND cal.client_id = cd.client_id
  );

-- Backfill: lead history (per client — same lead may map to multiple clients)
INSERT INTO public.client_activity_log (
...
WHERE lr.client_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.client_activity_log cal
    WHERE cal.source_table = 'lead_remarks' AND cal.source_id = lr.id AND cal.client_id = lr.client_id
  );

-- Backfill: client tasks
INSERT INTO public.client_activity_log (
  client_id, actor_id, actor_role, action, summary, new_value, metadata,
  source_table, source_id, created_at
)
