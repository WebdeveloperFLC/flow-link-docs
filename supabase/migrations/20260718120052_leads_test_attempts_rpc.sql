-- Phase D UAT: persist test_attempts on lead autosave (patch_lead_draft / create_lead_draft).
-- Migration 050 added columns; RPCs still omitted them so background tests vanished after save.
...
BEGIN
  ...
  INSERT INTO public.leads (
    ...
    COALESCE(_data -> 'test_attempts', '[]'::jsonb),
    COALESCE(_data -> 'active_attempt_ids', '{}'::jsonb),
...
BEGIN
  ...
  UPDATE public.leads l
     SET
       ...
       END,
       test_attempts = CASE
         WHEN _data ? 'test_attempts' THEN COALESCE(_data -> 'test_attempts', '[]'::jsonb)
         ELSE l.test_attempts
       END,
       active_attempt_ids = CASE
         WHEN _data ? 'active_attempt_ids' THEN COALESCE(_data -> 'active_attempt_ids', '{}'::jsonb)
         ELSE l.active_attempt_ids
       END,
