-- UAT Round 1: hook fn_apply into main demo seed (runs after base seed body)
...
BEGIN
  ...
  -- ── Fee schedules (ACTIVE) — upsert; institutions may already have default APPLICATION/TUITION rows ──
  WITH fee_seed AS (
    SELECT * FROM (VALUES
      ('d1e0c001-0001-4000-8000-000000000001'::uuid, v_seneca, 'APPLICATION', 90::numeric, 'CAD', 'EXACT', 'WEBSITE', DATE '2026-01-01', '[crm-direct-uat-v1] Seneca international application fee'),
      ('d1e0c002-0001-4000-8000-000000000002'::uuid, v_seneca, 'TUITION', 18000::numeric, 'CAD', 'APPROXIMATE', 'AGREEMENT', DATE '2026-01-01', '[crm-direct-uat-v1] Business PG Diploma — annual tuition guide'),
      ('d1e0c003-0001-4000-8000-000000000003'::uuid, v_seneca, 'DEPOSIT', 5000::numeric, 'CAD', 'EXACT', 'LOA', DATE '2026-01-01', '[crm-direct-uat-v1] Seneca deposit to secure seat'),
      ('d1e0c004-0002-4000-8000-000000000004'::uuid, v_conestoga, 'APPLICATION', 100::numeric, 'CAD', 'EXACT', 'WEBSITE', DATE '2026-01-01', '[crm-direct-uat-v1] Conestoga application fee'),
      ('d1e0c005-0002-4000-8000-000000000005'::uuid, v_conestoga, 'TUITION', 16800::numeric, 'CAD', 'APPROXIMATE', 'AGREEMENT', DATE '2026-01-01', '[crm-direct-uat-v1] Business diploma tuition guide'),
      ('d1e0c006-0003-4000-8000-000000000006'::uuid, v_humber, 'APPLICATION', 100::numeric, 'CAD', 'EXACT', 'WEBSITE', DATE '2026-01-01', '[crm-direct-uat-v1] Humber application fee'),
      ('d1e0c007-0003-4000-8000-000000000007'::uuid, v_humber, 'TUITION', 17500::numeric, 'CAD', 'APPROXIMATE', 'WEBSITE', DATE '2026-01-01', '[crm-direct-uat-v1] Hospitality Management tuition guide'),
      ('d1e0c008-0004-4000-8000-000000000008'::uuid, v_sheridan, 'APPLICATION', 95::numeric, 'CAD', 'EXACT', 'WEBSITE', DATE '2026-01-01', '[crm-direct-uat-v1] Sheridan application fee'),
      ('d1e0c009-0004-4000-8000-000000000009'::uuid, v_sheridan, 'TUITION', 19200::numeric, 'CAD', 'APPROXIMATE', 'AGREEMENT', DATE '2026-01-01', '[crm-direct-uat-v1] Animation Advanced Diploma tuition guide')
    ) AS v(id, inst_id, fee_type, amount, currency, fee_accuracy, verification_method, effective_from, note)
  ),
  updated AS (
    UPDATE public.institution_fee_schedule ifs
    SET
      amount = fs.amount,
      currency = fs.currency,
      fee_accuracy = fs.fee_accuracy,
      verification_method = fs.verification_method,
      effective_from = fs.effective_from,
      status = 'ACTIVE',
      notes = fs.note,
      updated_at = now()
    FROM fee_seed fs
    WHERE ifs.upi_institution_id = fs.inst_id
      AND ifs.fee_type = fs.fee_type
      AND ifs.status = 'ACTIVE'
      AND ifs.program_id IS NULL
      AND ifs.partnership_route_id IS NULL
    RETURNING ifs.id
  )
  ...
  SELECT
    fs.id, fs.inst_id, fs.fee_type, fs.amount, fs.currency, fs.fee_accuracy, fs.verification_method,
    fs.effective_from, 'ACTIVE', fs.note
  FROM fee_seed fs
  WHERE NOT EXISTS (
    SELECT 1 FROM public.institution_fee_schedule ifs
    WHERE ifs.upi_institution_id = fs.inst_id
      AND ifs.fee_type = fs.fee_type
      AND ifs.status = 'ACTIVE'
      AND ifs.program_id IS NULL
  ...
  UPDATE public.upi_commission_students SET commission_snapshot_id = v_snap_priya WHERE id = v_stu_priya;
  UPDATE public.upi_commission_students SET commission_snapshot_id = v_snap_ananya WHERE id = v_stu_ananya;
  UPDATE public.upi_commission_students SET commission_snapshot_id = v_snap_fatima WHERE id = v_stu_fatima;
  UPDATE public.upi_commission_students SET commission_snapshot_id = v_snap_ahmed_tgt WHERE id = v_stu_ahmed_tgt;
  ...
  UPDATE public.upi_commission_students SET invoice_id = v_inv_seneca WHERE id = v_stu_priya;
  UPDATE public.upi_commission_students SET invoice_id = v_inv_conestoga WHERE id = v_stu_fatima;
  ...
  UPDATE public.upi_commission_invoices
  SET last_receipt_id = v_rcpt_partial
  WHERE id = v_inv_seneca;

  PERFORM public.fn_apply_commission_demo_uat_templates();
