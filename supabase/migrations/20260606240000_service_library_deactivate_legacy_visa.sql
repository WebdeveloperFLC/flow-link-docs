-- Deactivate remaining legacy visa rows that duplicate country names (sidebar clutter).
-- Preserves rows referenced in leads/clients service arrays.
...
WHERE sl.service_category = 'visa_immigration'
  ...
  AND (
    lower(sl.sub_service) IN ('application', 'assessment')
