-- Copy test_attempts + active_attempt_ids on lead → client conversion (Phase E parity).
...
BEGIN
  ...
  v_year_date := CASE
    WHEN v_e0 IS NULL THEN NULL
    WHEN NULLIF(substring(COALESCE(v_e0 ->> 'end_year', v_e0 ->> 'year') from '\d{4}'), '') IS NULL THEN NULL
    ELSE make_date(
      NULLIF(substring(COALESCE(v_e0 ->> 'end_year', v_e0 ->> 'year') from '\d{4}'), '')::int,
      6,
      30
    )
  END;
