-- Institution Master remediation: shell UPI records for 12 unlinked CF universities.
-- Creates upi_institutions (no fee schedules, commissions, or partnership routes).
-- Links cf_universities.upi_institution_id to exact-name matches.
...
BEGIN
  ...
  -- Create shell UPI institutions (idempotent on dedup key).
  WITH inserted AS (
  ...
  -- Link CF universities to matching UPI shell (exact name + country).
  WITH updated AS (
  ...
  -- Supersede stale unmatched linkage candidates for remediated CF rows.
...
-- Post-verify: all seed CF rows must be linked.
DO $$
DECLARE
  v_unlinked integer;
BEGIN
