-- F3.4 catch-up: aggregator invoice RLS when Phase 2B tables exist.
-- Safe if 20261030120100 already skipped missing Phase 2B tables.
-- Also replaces Phase 2B ucai_confidential / ucail_confidential if published after F3.4.
...
BEGIN
  IF to_regclass('public.upi_commission_aggregator_invoice_lines') IS NOT NULL THEN
    ...
    CREATE POLICY uc_fin_upi_commission_aggregator_invoice_lines_select
      ON public.upi_commission_aggregator_invoice_lines
      FOR SELECT TO authenticated
      USING (public.can_view_commission_financial(auth.uid(), institution_id));
    CREATE POLICY uc_fin_upi_commission_aggregator_invoice_lines_insert
      ON public.upi_commission_aggregator_invoice_lines
      FOR INSERT TO authenticated
      WITH CHECK (public.can_manage_commission_financial(auth.uid(), institution_id));
    CREATE POLICY uc_fin_upi_commission_aggregator_invoice_lines_update
      ON public.upi_commission_aggregator_invoice_lines
      FOR UPDATE TO authenticated
      USING (public.can_manage_commission_financial(auth.uid(), institution_id))
      WITH CHECK (public.can_manage_commission_financial(auth.uid(), institution_id));
    CREATE POLICY uc_fin_upi_commission_aggregator_invoice_lines_delete
      ON public.upi_commission_aggregator_invoice_lines
