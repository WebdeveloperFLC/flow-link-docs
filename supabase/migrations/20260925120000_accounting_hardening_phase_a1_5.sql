-- Accounting Hardening Phase A1.5
-- Pre-financial vs financial tiers; service-scoped draft cleanup on removal.

-- ---------------------------------------------------------------------------
-- 1. Invoice cancellation metadata (soft cancel — no hard delete)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Extend service audit log actions
-- ---------------------------------------------------------------------------
...
AS $$
  SELECT
    COALESCE(p_line->>'service_id', '') = ANY (p_keys)
...
AS $$
  SELECT COALESCE(SUM(
    CASE
      WHEN public.fn_jsonb_line_is_billable(li)
      THEN COALESCE((li->>'total')::numeric, 0)
      ELSE 0
    END
  ), 0)
...
-- ---------------------------------------------------------------------------
-- 4. Dependency assessment — pre_financial vs financial tiers (Phase A1.5)
-- ---------------------------------------------------------------------------
...
BEGIN
  v_keys := COALESCE(
    NULLIF(p_match_keys, ARRAY[]::text[]),
    ARRAY[p_service_code]
  );

  SELECT COALESCE(array_agg(c.id), ARRAY[]::uuid[])
  INTO v_open_case_ids
  FROM public.client_service_cases c
  WHERE c.client_id = p_client_id
    AND c.status = 'open'
    AND (
      c.service_code = ANY (v_keys)
      OR split_part(c.service_code, '::', 1) = ANY (v_keys)
    );

  -- All active invoices touching this service (any status except terminal)
  SELECT COALESCE(array_agg(i.id), ARRAY[]::uuid[])
  INTO v_service_invoice_ids
  ...
  SELECT COALESCE(array_agg(i.id), ARRAY[]::uuid[])
  INTO v_draft_invoice_ids
  FROM public.client_invoices i
  WHERE i.id = ANY (COALESCE(v_service_invoice_ids, ARRAY[]::uuid[]))
    AND i.status = 'draft';

  SELECT COALESCE(array_agg(i.id), ARRAY[]::uuid[])
  INTO v_issued_invoice_ids
  FROM public.client_invoices i
  WHERE i.id = ANY (COALESCE(v_service_invoice_ids, ARRAY[]::uuid[]))
    AND i.status <> 'draft';
  ...
  IF COALESCE(array_length(v_service_invoice_ids, 1), 0) > 0 THEN
    ...
    WHERE p.invoice_id = ANY (v_service_invoice_ids)
      AND p.archived_at IS NULL;

    SELECT COUNT(*)::int, COALESCE(SUM(a.amount_allocated), 0)
    INTO v_allocations_count, v_allocations_amount
    FROM public.client_invoice_payment_allocations a
    WHERE a.invoice_id = ANY (v_service_invoice_ids)
      AND a.service_id::text = ANY (v_keys);

    SELECT COUNT(*)::int INTO v_receipts_count
    FROM public.client_invoice_receipts r
    WHERE r.invoice_id = ANY (v_service_invoice_ids)
      AND r.archived_at IS NULL
      AND COALESCE(r.receipt_voided, false) = false;

    SELECT COUNT(*)::int INTO v_refunds_count
    FROM public.client_invoice_refund_requests rr
    WHERE rr.invoice_id = ANY (v_service_invoice_ids)
      AND rr.archived_at IS NULL;

    SELECT COUNT(*)::int INTO v_adjustments_count
    FROM public.client_invoice_adjustments adj
    WHERE adj.invoice_id = ANY (v_service_invoice_ids)
      AND adj.archived_at IS NULL
      AND adj.status = 'applied';

    -- Informational only — line/offer discounts do not block pre-financial
    SELECT COUNT(*)::int INTO v_discounts_count
    FROM public.client_invoices i
    ...
    SELECT COUNT(*)::int INTO v_wallet_count
    FROM public.wallet_allocations wa
    WHERE wa.client_id = p_client_id
      AND wa.invoice_id = ANY (v_service_invoice_ids)
      AND wa.status IN ('applied', 'reserved');

    SELECT COUNT(DISTINCT b.journal_id)::int INTO v_journals_count
    FROM public.accounting_crm_invoice_bridge b
    WHERE b.invoice_id = ANY (v_service_invoice_ids)
      AND b.journal_id IS NOT NULL;

    SELECT COUNT(*)::int INTO v_trust_count
    FROM public.accounting_trust_entries te
    JOIN public.accounting_crm_invoice_bridge b ON b.journal_id = te.journal_id
    WHERE b.invoice_id = ANY (v_service_invoice_ids);
  END IF;

  -- Allocations on service_id without line match on invoice set
  ...
  -- Issued/sent invoice = financial (decision #8)
  v_has_financial := v_issued_count > 0
    OR v_payments_count > 0
    OR v_allocations_count > 0
    OR v_receipts_count > 0
    OR v_refunds_count > 0
    OR v_adjustments_count > 0
    OR v_wallet_count > 0
    OR v_journals_count > 0
    OR v_trust_count > 0;

  -- Any service-linked invoice with amount_paid > 0
  ...
  IF COALESCE(array_length(v_open_case_ids, 1), 0) > 0 THEN
    ...
    WHERE d.client_id = p_client_id
      ...
      AND d.case_id = ANY (v_open_case_ids);

    SELECT COUNT(*)::int INTO v_forms_count
    FROM public.client_documents d
    WHERE d.client_id = p_client_id
      AND d.deleted_at IS NULL
      AND d.case_id = ANY (v_open_case_ids)
      AND (
        COALESCE(d.document_type, '') ILIKE '%form%'
        OR COALESCE(d.custom_type, '') ILIKE '%form%'
      );
  END IF;

  SELECT COUNT(*)::int INTO v_tasks_count
  FROM public.client_tasks t
  WHERE t.client_id = p_client_id
    AND t.status IN ('open', 'in_progress');

  SELECT COUNT(*)::int INTO v_notes_count
  FROM public.client_timeline tl
...
-- ---------------------------------------------------------------------------
-- 5. Service-scoped pre-financial draft cleanup
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  FOR v_inv IN
    ...
    WHERE i.client_id = p_client_id
      ...
      AND public.fn_invoice_line_matches_service_keys(i.line_items, v_keys)
  LOOP
    -- Per-invoice pre-financial guard
    IF EXISTS (
      SELECT 1 FROM public.client_invoice_payments p
      WHERE p.invoice_id = v_inv.id AND p.archived_at IS NULL
    ) THEN
      RAISE EXCEPTION 'Draft invoice % has payments — cleanup blocked', v_inv.invoice_number;
    END IF;

    IF COALESCE((
      SELECT amount_paid FROM public.client_invoices WHERE id = v_inv.id
    ), 0) > 0 THEN
    ...
    IF v_billable_matching = 0 THEN
      CONTINUE;
    END IF;

    IF v_billable_remaining = 0 THEN
      -- Entire invoice is for this service — cancel and hide
      ...
      SET status = 'cancelled',
          ...
          amount = 0,
          balance_due_in_inr = 0,
          balance_due_in_cad = 0,
          balance_due_in_usd = 0
      WHERE id = v_inv.id;

      v_cancelled := array_append(v_cancelled, v_inv.id);
    ELSE
      -- Mixed invoice — strip service lines; keep other services + checkout meta
      FOR v_line IN SELECT value FROM jsonb_array_elements(v_lines)
      LOOP
      ...
      UPDATE public.client_invoices
      SET line_items = v_new_lines,
          amount = v_new_amount,
          subtotal_in_inr = v_new_amount,
          subtotal_in_cad = v_new_amount,
          subtotal_in_usd = v_new_amount,
          balance_due_in_inr = v_new_amount,
          balance_due_in_cad = v_new_amount,
          balance_due_in_usd = v_new_amount,
