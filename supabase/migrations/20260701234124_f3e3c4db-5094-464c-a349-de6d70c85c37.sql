...
DO $$ BEGIN
  CREATE TYPE wtm_payroll_status AS ENUM ('Present', 'Half Day', 'Absent', 'Paid Leave', 'Unpaid Leave', 'Holiday', 'Weekly Off');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE wre_eval_trigger AS ENUM ('clock_out', 'aems_correction', 'manual_reeval', 'policy_change', 'calendar_change');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.wre_evaluations TO authenticated;
GRANT ALL ON public.wre_evaluations TO service_role;
CREATE INDEX IF NOT EXISTS idx_wre_evaluations_session ON wre_evaluations (session_id, evaluated_at DESC);
CREATE INDEX IF NOT EXISTS idx_wre_evaluations_emp_month ON wre_evaluations (employee_id, work_date DESC);
ALTER TABLE wre_evaluations ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS wre_evaluations_select ON wre_evaluations;
CREATE POLICY wre_evaluations_select ON wre_evaluations FOR SELECT USING (is_hr(org_id) OR manages_employee(org_id, employee_id) OR employee_id = current_employee_id(org_id));
DROP POLICY IF EXISTS wre_evaluations_insert ON wre_evaluations;
CREATE POLICY wre_evaluations_insert ON wre_evaluations FOR INSERT WITH CHECK (current_hr_role(org_id) IS NOT NULL OR employee_id = current_employee_id(org_id));
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.wtm_attendance_snapshots TO authenticated;
GRANT ALL ON public.wtm_attendance_snapshots TO service_role;
CREATE INDEX IF NOT EXISTS idx_wtm_snapshots_emp_date ON wtm_attendance_snapshots (employee_id, work_date DESC);
CREATE INDEX IF NOT EXISTS idx_wtm_snapshots_org_month ON wtm_attendance_snapshots (org_id, work_date);
ALTER TABLE wtm_attendance_snapshots ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS wtm_snapshots_select ON wtm_attendance_snapshots;
CREATE POLICY wtm_snapshots_select ON wtm_attendance_snapshots FOR SELECT USING (is_hr(org_id) OR manages_employee(org_id, employee_id) OR employee_id = current_employee_id(org_id));
DROP POLICY IF EXISTS wtm_snapshots_insert ON wtm_attendance_snapshots;
CREATE POLICY wtm_snapshots_insert ON wtm_attendance_snapshots FOR INSERT WITH CHECK (current_hr_role(org_id) IS NOT NULL OR employee_id = current_employee_id(org_id));
...
BEGIN
  SELECT bundle_id INTO v_bundle FROM wpms_employee_bundle_assignments
  WHERE employee_id = p_employee AND effective_from <= p_as_of AND (effective_to IS NULL OR effective_to >= p_as_of)
  ORDER BY effective_from DESC LIMIT 1;
  IF v_bundle IS NULL THEN SELECT wpms_current_bundle_id INTO v_bundle FROM employees WHERE id = p_employee; END IF;
  RETURN v_bundle;
END; $$;
GRANT EXECUTE ON FUNCTION fn_wpms_employee_bundle_at TO authenticated;

CREATE OR REPLACE FUNCTION fn_wpms_policy_config_at(
  p_employee uuid, p_kind wpms_policy_kind, p_as_of date DEFAULT CURRENT_DATE
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE v_bundle uuid; v_policy_id uuid; v_config jsonb;
BEGIN
  ...
  SELECT CASE p_kind
    ...
    ELSE NULL END INTO v_policy_id
...
BEGIN
  ...
  ELSE
    ...
    v_payroll := CASE (v_derived->>'status')
      ...
      ELSE 'Absent'::wtm_payroll_status END;
  ...
  VALUES (s.org_id, s.employee_id, s.id, s.work_date, p_trigger, p_trigger_ref,
    ...
    v_bundle_row.version, v_att_policy.version, v_hol_policy_row.version, v_policy,
  ...
  VALUES (s.org_id, s.id, s.employee_id, s.work_date, v_eval.id, v_version,
    ...
    v_payroll, v_operational, v_mispunch, v_bundle_row.version, v_att_policy.version, v_hol_policy_row.version);
  ...
  PERFORM fn_wtm_log_event(s.org_id, s.employee_id,
    ...
    jsonb_build_object('evaluation_id', v_eval.id, 'payroll_status', v_payroll,
      'late_minutes', v_late, 'remaining_grace', v_remaining_grace, 'policy_version', v_att_policy.version),
...
CREATE OR REPLACE FUNCTION fn_wre_reevaluate(
  p_org uuid, p_from date, p_to date, p_employee uuid DEFAULT NULL,
...
DECLARE v_ids uuid[]; v_id uuid; v_count int := 0;
BEGIN
  IF NOT (has_perm(p_org, 'manage_emp') OR has_perm(p_org, 'approve')) THEN RAISE EXCEPTION 'Not authorized'; END IF;
  SELECT array_agg(id) INTO v_ids FROM wtm_attendance_sessions
  WHERE org_id = p_org AND work_date BETWEEN p_from AND p_to
  ...
  IF p_dry_run THEN RETURN jsonb_build_object('dry_run', true, 'count', array_length(v_ids, 1), 'session_ids', v_ids); END IF;
...
DECLARE s wtm_attendance_sessions; v_time time := COALESCE(p_time, current_time); v_now timestamptz := now(); v_open_break int;
BEGIN
  ...
  PERFORM fn_wtm_log_event(s.org_id, s.employee_id, 'AttendanceSessionCompleted', s.id, NULL,
    jsonb_build_object('work_date', s.work_date, 'new', v_time::text, 'working_min', s.working_duration_min, 'payroll_status', s.payroll_status),
    p_actor_id, p_actor_label);
