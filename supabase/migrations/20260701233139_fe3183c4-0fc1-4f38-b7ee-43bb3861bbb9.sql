...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT COALESCE(
    (
      ...
      WHERE p.org_id = p_org AND p.domain = 'payroll'
...
DECLARE
  ...
  v_off_shift int := 0; v_shift_work int := 0; v_late_exempt int := 0; v_mis_approved int := 0;
BEGIN
  SELECT * INTO c FROM payroll_cycles WHERE id = p_cycle;
  SELECT * INTO e FROM employees WHERE id = p_employee;

  SELECT
    count(*) FILTER (WHERE a.status = 'Present') + count(*) FILTER (WHERE a.status = 'Half Day') * 0.5,
    ...
    count(*) FILTER (WHERE a.is_mispunch) + count(*) FILTER (WHERE a.status = 'Absent'),
    count(*) FILTER (
      WHERE fn_is_late_check_in(a.check_in, COALESCE(sh.login_time, '10:00'::time),
        COALESCE(sh.logout_time, '19:00'::time), COALESCE(sh.grace_min, 5))
  ...
  WHERE le.employee_id = p_employee AND le.status = 'Approved'
    AND le.late_date BETWEEN c.start_date AND c.end_date
    AND le.delay_min > COALESCE(
      (SELECT s.grace_min FROM shifts s WHERE s.id = fn_employee_shift_at(p_employee, le.late_date)), 5);
  ...
  FROM leave_requests WHERE employee_id = p_employee AND status = 'Approved'
    AND from_date BETWEEN c.start_date AND c.end_date;
  ...
  RETURN jsonb_build_object(
    ...
    'leaves', v_leaves, 'paid_leaves', v_paid, 'comp_off', v_compoff, 'ul', v_ul,
    'sandwich', v_sandwich, 'unpaid_training', v_train, 'ot_minutes', v_ot,
    'off_shift_minutes', v_off_shift, 'shift_work_minutes', v_shift_work,
    'working', round(v_working, 1), 'week_off', v_woff
  );
END;
$$;
...
DECLARE
  v_eligible boolean; v_att record; v_status att_status;
  v_base numeric := 0; v_bonus numeric := 0; v_day numeric := 0;
  v_is_wo boolean; v_worked_wo_hol boolean := false;
  v_half_leave boolean := false; v_paid_leave numeric := 0;
BEGIN
  v_eligible := fn_employee_attendance_eligible(p_employee, p_date);
  IF NOT v_eligible THEN
    RETURN jsonb_build_object('base_credit', 0, 'comp_off_bonus', 0, 'day_credit', 0,
      'late_eligible', false, 'mispunch_eligible', false, 'ul_day', false);
  END IF;

  SELECT a.status, a.is_mispunch, a.check_in, a.check_out INTO v_att
  FROM attendance a WHERE a.employee_id = p_employee AND a.work_date = p_date;

  v_status := v_att.status;
  IF v_status IS NULL THEN v_status := 'Absent'; END IF;
  v_is_wo := fn_is_weekly_off_day(p_org, p_employee, p_date);

  SELECT EXISTS (
    SELECT 1 FROM leave_requests lr WHERE lr.employee_id = p_employee
      AND lr.status = 'Approved' AND lr.type <> 'Unpaid Leave'
      AND p_date BETWEEN lr.from_date AND lr.to_date
      AND (lr.duration_type = 'Half Day' OR lr.days <= 0.5)
  ) INTO v_half_leave;

  IF v_status = 'Holiday' THEN v_base := 1.0;
  ELSIF v_half_leave AND v_status = 'Half Day' THEN v_base := 1.0;
  ELSIF v_status = 'Week Off' THEN v_base := 1.0;
  ELSIF v_status = 'Present' THEN v_base := 1.0; v_worked_wo_hol := v_is_wo OR v_status = 'Holiday';
  ELSIF v_status = 'Half Day' THEN v_base := 0.5; v_worked_wo_hol := v_is_wo;
  ELSIF v_status IN ('Leave', 'Sick Leave') THEN v_base := 1.0;
  ELSE
    SELECT COALESCE(max(CASE WHEN lr.duration_type = 'Half Day' OR lr.days <= 0.5 THEN 0.5 ELSE 1.0 END), 0)
    INTO v_paid_leave FROM leave_requests lr
    WHERE lr.employee_id = p_employee AND lr.status = 'Approved' AND lr.type <> 'Unpaid Leave'
      AND p_date BETWEEN lr.from_date AND lr.to_date;
    IF v_paid_leave > 0 THEN v_base := v_paid_leave;
    ELSIF v_status = 'Unauthorized Leave' THEN v_base := 0;
    ELSE v_base := 0; END IF;
  END IF;

  IF v_status = 'Present' AND (v_is_wo OR v_status = 'Holiday') THEN v_worked_wo_hol := true; END IF;
  IF v_status = 'Half Day' AND v_is_wo THEN v_worked_wo_hol := true; END IF;

  IF v_worked_wo_hol AND EXISTS (
    SELECT 1 FROM compoff_requests co WHERE co.employee_id = p_employee
      AND co.status = 'Approved' AND co.worked_date = p_date
  ) THEN v_bonus := least(1.0, 2.0 - v_base); END IF;

  v_day := least(v_base + v_bonus, 2.0);

  RETURN jsonb_build_object(
    'base_credit', v_base, 'comp_off_bonus', v_bonus, 'day_credit', round(v_day, 2),
    'late_eligible', (v_att.check_in IS NOT NULL AND v_status IS NOT NULL
      AND v_status NOT IN ('Week Off', 'Holiday', 'Leave', 'Sick Leave', 'Unauthorized Leave', 'Absent', 'Half Day')
      AND fn_is_late_check_in(v_att.check_in,
...
DECLARE c payroll_cycles; e employees; v_from date; v_to date; v_eligible int;
BEGIN
  SELECT * INTO c FROM payroll_cycles WHERE id = p_cycle;
  SELECT * INTO e FROM employees WHERE id = p_employee;
  ...
  IF v_from = c.start_date AND v_to = c.end_date THEN RETURN c.payroll_days; END IF;
...
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  c record; e record; v_pol jsonb; v_from date; v_to date; d date; v_day jsonb;
  v_attendance_earned numeric := 0; v_late int := 0; v_mis int := 0; v_ul int := 0;
  v_late_exempt int := 0; v_mis_approved int := 0; v_sandwich numeric := 0;
  v_train int := 0; v_ot int := 0; v_off_shift int := 0; v_shift_work int := 0;
  v_compoff_audit int := 0; v_effective int; v_breakdown jsonb := '{}'::jsonb;
  v_present int := 0; v_half_count int := 0; v_wo int := 0; v_holiday int := 0;
  v_comp_bonus numeric := 0; v_day_present int; v_day_half numeric; v_day_wo int; v_day_hol int;
BEGIN
  SELECT * INTO c FROM payroll_cycles WHERE id = p_cycle;
  SELECT * INTO e FROM employees WHERE id = p_employee;
  ...
  WHILE d <= v_to LOOP
    ...
    IF COALESCE((v_day->>'late_eligible')::boolean, false) THEN v_late := v_late + 1; END IF;
    IF COALESCE((v_day->>'mispunch_eligible')::boolean, false) THEN v_mis := v_mis + 1; END IF;
    IF COALESCE((v_day->>'ul_day')::boolean, false) THEN v_ul := v_ul + 1; END IF;

    SELECT CASE WHEN a.status = 'Present' THEN 1 ELSE 0 END,
    ...
    FROM attendance a WHERE a.employee_id = p_employee AND a.work_date = d;
  ...
  FROM attendance a
  LEFT JOIN shifts sh ON sh.id = fn_employee_shift_at(a.employee_id, a.work_date)
  WHERE a.employee_id = p_employee AND a.work_date BETWEEN v_from AND v_to
    AND a.status IN ('Present', 'Half Day');

  v_breakdown := jsonb_build_object(
    'present', v_present, 'half', v_half_count, 'week_off', v_wo, 'holiday', v_holiday,
    'comp_off_bonus', round(v_comp_bonus, 2), 'comp_off_audit_count', v_compoff_audit
  );
  ...
  RETURN jsonb_build_object(
    ...
    'late', greatest(0, v_late - v_late_exempt),
    'mispunch', greatest(0, v_mis - v_mis_approved),
    'leaves', 0, 'paid_leaves', 0,
    'comp_off', v_compoff_audit, 'ul', v_ul, 'sandwich', v_sandwich,
    'unpaid_training', v_train, 'ot_minutes', v_ot,
    'off_shift_minutes', v_off_shift, 'shift_work_minutes', v_shift_work,
    'working', round(v_attendance_earned, 1), 'week_off', v_wo
  );
END;
$$;
...
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE c payroll_cycles; e employees; v_mode text;
BEGIN
  SELECT * INTO c FROM payroll_cycles WHERE id = p_cycle;
  SELECT * INTO e FROM employees WHERE id = p_employee;
  v_mode := fn_payroll_formula_mode(e.org_id, c.start_date);
  IF v_mode = 'earned' THEN RETURN fn_rollup_inputs_earned(p_employee, p_cycle); END IF;
...
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE v_out jsonb := '{}'::jsonb; d text; pol jsonb;
BEGIN
  FOR d IN SELECT unnest(ARRAY[
    ...
    IF pol IS NOT NULL THEN v_out := v_out || jsonb_build_object(d, pol); END IF;
  END LOOP;
  RETURN v_out;
END;
$$;
...
-- =====================================================================
-- Salary structure & statutory columns
-- =====================================================================
...
SET employer_pf_applicable = pf_applicable, employer_esic_applicable = esic_applicable
WHERE employer_pf_applicable = false AND employer_esic_applicable = false
...
-- =====================================================================
-- Structure resolver + gate
-- =====================================================================

CREATE OR REPLACE FUNCTION fn_resolve_employee_salary_structure(
  p_employee uuid, p_pt_default numeric DEFAULT 200
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  e employees%ROWTYPE;
  v_basic numeric; v_hra numeric; v_conveyance numeric;
  v_bonus_pct numeric; v_bonus_amount numeric; v_other numeric;
  v_total_a numeric; v_pf_wage numeric;
  v_pf_ceiling numeric := 15000; v_esic_ceiling numeric := 21000;
  v_employer_pf numeric := 0; v_employer_esic numeric := 0;
  v_total_b numeric := 0; v_diff numeric; v_pt_amount numeric; v_salary_package numeric;
BEGIN
  SELECT * INTO e FROM employees WHERE id = p_employee;
  IF e.id IS NULL THEN RAISE EXCEPTION 'Employee not found: %', p_employee; END IF;
  ...
  v_pt_amount := CASE WHEN COALESCE(e.pt_applicable, false)
    THEN COALESCE(e.professional_tax_amount, p_pt_default) ELSE 0 END;
  ...
  IF COALESCE(e.employer_esic_applicable, e.esic_applicable)
     AND COALESCE(e.esic_applicable, false) AND v_total_a <= v_esic_ceiling THEN
  ...
  RETURN jsonb_build_object(
    'salary_package', v_salary_package, 'basic', v_basic, 'hra', v_hra,
    'conveyance', v_conveyance, 'bonus_percentage', v_bonus_pct, 'bonus_amount', v_bonus_amount,
    'other_allowances', v_other, 'total_earnings_a', v_total_a,
    ...
    'pt_amount', v_pt_amount, 'employer_pf', v_employer_pf, 'employer_esic', v_employer_esic,
    'total_employer_cost_b', v_total_b, 'structure_difference', v_diff
  );
END;
$$;
...
-- =====================================================================
-- Salary Payable Days engine + final fn_compute_payroll + fn_build_payroll_line
-- (final Phase 2A form: attendance → payable → salary from monthly_gross)
-- =====================================================================
...
DECLARE
  k numeric; n numeric; payable numeric; v_formula text;
  v_divisor int; v_ul_ded numeric; v_att_earned numeric;
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'late_count', COALESCE(p_late, 0), 'late_deduction', k,
    'mispunch_count', COALESCE(p_mispunch, 0), 'mispunch_deduction', n,
    'ul_count', COALESCE(p_ul, 0), 'ul_deduction_days', v_ul_ded,
...
DECLARE
  v_payable jsonb; payable numeric; daily numeric; gross numeric;
  ...
  v_ot_pay numeric := 0; v_hourly numeric;
  v_ot_mode text; v_min_ot int;
  v_cpp_rate numeric; v_ei_rate numeric; v_policy_other numeric;
  v_is_ca boolean; v_divisor int; k numeric; n numeric;
  v_factor numeric; v_struct_basic numeric; v_struct_hra numeric;
  v_struct_conveyance numeric; v_struct_bonus numeric; v_struct_other numeric;
  v_er_pf numeric := 0; v_er_esic numeric := 0;
  v_emp_pf_pct numeric; v_emp_esic_pct numeric; v_er_pf_pct numeric; v_er_esic_pct numeric;
  v_monthly_b numeric; v_diff numeric; v_esic_monthly numeric; v_monthly_a numeric;
BEGIN
  ...
  IF v_ot_mode = 'paid' AND COALESCE(p_ot_minutes, 0) >= v_min_ot THEN
    ...
    v_ot_pay := round((p_ot_minutes / 60.0) * v_hourly *
      COALESCE((p_ot_policy->>'rate_multiplier')::numeric, 1.5));
  END IF;
  ...
  IF p_structure_enabled AND NOT v_is_ca AND p_structure IS NOT NULL THEN
    ...
    esic_emp := CASE WHEN p_esic_applicable AND v_esic_monthly <= esic_ceiling
      THEN round(gross * v_emp_esic_pct / 100) ELSE 0 END;
    pt_emp := CASE WHEN COALESCE(p_pt_applicable, false)
      THEN COALESCE((p_structure->>'pt_amount')::numeric, p_pt_amount) ELSE 0 END;
    IF COALESCE(p_tds_applicable, false) THEN pt_emp := pt_emp + COALESCE(p_other_deductions, 0); END IF;
    ...
    RETURN jsonb_build_object(
      ...
      'late_deduction', k, 'mispunch_deduction', n,
      'payable_days', round(payable, 2), 'daily_rate', daily, 'gross_earned', gross,
      'pf_employee', pf_emp, 'esic_employee', esic_emp, 'pt_employee', pt_emp,
      'incentive', COALESCE(p_incentive, 0), 'bonus', 0,
      'ot_minutes', COALESCE(p_ot_minutes, 0), 'ot_pay', v_ot_pay,
      'net_salary', round(net), 'payroll_country', 'IN',
      ...
      'structure_basic', v_struct_basic, 'structure_hra', v_struct_hra,
      'structure_conveyance', v_struct_conveyance, 'structure_bonus', v_struct_bonus,
      'structure_other_allowances', v_struct_other, 'total_earnings_a', gross,
      'employer_pf', v_er_pf, 'employer_esic', v_er_esic,
      'total_employer_cost_b', v_monthly_b, 'structure_difference', v_diff
    );
  END IF;
  ...
  ELSE
    ...
    esic_emp := CASE WHEN p_esic_applicable AND p_monthly <= esic_ceiling
      THEN round(gross * 0.0075) ELSE 0 END;
  ...
  RETURN jsonb_build_object(
    ...
    'payable_days', round(payable, 2), 'daily_rate', daily, 'gross_earned', gross,
    'pf_employee', pf_emp, 'esic_employee', esic_emp, 'pt_employee', pt_emp,
    'incentive', COALESCE(p_incentive, 0), 'bonus', COALESCE(p_bonus, 0),
    'ot_minutes', COALESCE(p_ot_minutes, 0), 'ot_pay', v_ot_pay,
...
DECLARE
  ...
  v_pay_pol jsonb; v_ul_mult numeric; v_pt_amt numeric; v_country text;
  v_formula_mode text; v_effective int; v_earned numeric;
  v_structure jsonb; v_structure_enabled boolean; v_cycle_bonus numeric;
BEGIN
  SELECT * INTO c FROM payroll_cycles WHERE id = p_cycle;
  ...
  v_structure := CASE WHEN v_structure_enabled
    THEN fn_resolve_employee_salary_structure(p_employee, v_pt_amt) ELSE NULL END;
  ...
  calc := fn_compute_payroll(
    ...
    v_formula_mode, v_effective, v_earned, v_structure, v_structure_enabled
  );
  ...
  INSERT INTO payroll_lines AS pl (
    ...
    calc, calc->'payable_days_breakdown'
  )
...
DECLARE
  c payroll_cycles; e record;
  v_legacy jsonb; v_earned jsonb; v_legacy_calc jsonb; v_earned_calc jsonb;
  v_rows jsonb := '[]'::jsonb;
  v_late_pol jsonb; v_mis_pol jsonb; v_sul_pol jsonb; v_ot_pol jsonb; v_pt_pol jsonb; v_ca_pol jsonb;
...
BEGIN
  ...
  FOR e IN SELECT emp.* FROM employees emp
  ...
  LOOP
    ...
    v_legacy_calc := fn_compute_payroll(
      ...
      CASE WHEN COALESCE(e.pt_applicable, false) THEN v_pt_amt ELSE 0 END,
      v_country, v_ca_pol, COALESCE(e.other_deductions, 0), COALESCE(e.tds_applicable, false),
      'legacy', NULL, NULL, NULL, false
    );

    v_earned_calc := fn_compute_payroll(
      c.payroll_days, e.monthly_gross, e.basic, e.incentive, e.bonus,
      e.pf_applicable, e.esic_applicable, 0, 0,
    ...
    v_rows := v_rows || jsonb_build_array(jsonb_build_object(
      ...
      'delta', round(
        ...
        - COALESCE((v_legacy_calc->>'payable_days')::numeric, 0), 2),
  ...
  RETURN jsonb_build_object(
    'cycle_id', p_cycle, 'cycle_label', c.label,
...
BEGIN
  FOR r IN
    ...
    WHERE p.proname IN (
      'fn_resolve_payroll_policy', 'fn_payroll_formula_mode',
      'fn_rollup_inputs_legacy', 'fn_rollup_inputs_earned', 'fn_rollup_inputs',
      'fn_apply_priority_matrix_c17', 'fn_payroll_days_effective',
      'fn_compute_payroll', 'fn_build_payroll_line',
      'fn_compare_payroll_formulas', 'fn_collect_cycle_policies',
      'fn_resolve_employee_salary_structure', 'fn_employee_salary_structure_enabled',
