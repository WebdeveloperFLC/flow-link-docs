...
export async function fetchStudentFinancialSummary(clientId: string): Promise<StudentFinancialSummary | null> {
  ...
  return {
    ...
    services: (d.services ?? []).map((s: any) => ({
      ...
      requested: s.requested != null ? Number(s.requested) : null,
      requestedCurrency: s.requested_currency ?? null,
      institutionRequiredDeposit:
        s.institution_required_deposit != null ? Number(s.institution_required_deposit) : null,
      billingTrigger: s.billing_trigger ?? null,
      institutionDepositReference: s.institution_deposit_reference ?? null,
      invoiced: Number(s.invoiced) || 0,
      collected: Number(s.collected) || 0,
      outstanding: Number(s.outstanding) || 0,
      remainingBillable: s.remaining_billable != null ? Number(s.remaining_billable) : null,
      trustHeld: Number(s.trust_held) || 0,
      collectionStatus: s.collection_status || "NOT_INVOICED",
      currency: s.currency || s.requested_currency || "INR",
      invoicesByStage: (s.invoices_by_stage ?? []).map((row: any) => ({
        invoiceId: row.invoice_id,
        invoiceNumber: row.invoice_number ?? "",
        billingStage: row.billing_stage ?? "INSTALLMENT",
