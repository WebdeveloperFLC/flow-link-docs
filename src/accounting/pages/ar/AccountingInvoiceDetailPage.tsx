...
export default function AccountingInvoiceDetailPage() {
  ...
  const bank = useMemo(
    () => (inv?.linkedBankAccountId ? SEED_BANK_ACCOUNTS.find((b) => b.id === inv.linkedBankAccountId) : null),
    [inv],
  );
      ...
      <AppLayout>
        <div className="p-6">
          <AccountingEmptyState
            ...
            action={
              <Button onClick={() => navigate("/accounting/ar")}>
  ...
  const aging =
    inv.status === "PAID"
      ? {
          ...
          text: `Paid in full on ${inv.paidDate}`,
        }
      : inv.status === "OVERDUE"
        ? {
            tone: "bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400",
            text: `This invoice is ${inv.daysOverdue ?? Math.abs(due)} days overdue`,
          }
        : inv.status === "PARTIALLY_PAID"
          ? {
              tone: "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400",
              text: `Partially paid — ${fmtMoney(inv.outstandingBalance, inv.currency)} outstanding`,
            }
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-5">
        <div className="sticky top-0 bg-background z-10 -mx-6 px-6 py-3 border-b border-border">
          <AccountingPageHeader
            ...
            subtitle={`AR — Invoices / ${inv.invoiceNumber}`}
            actions={
              <>
                ...
                {inv.status === "DRAFT" && (
                  <Button
                    variant="outline"
                    onClick={() => {
                      updateArInvoice(inv.id, { status: "SENT" });
                ...
                {(inv.status === "SENT" || inv.status === "OVERDUE" || inv.status === "PARTIALLY_PAID") && (
                  <Button
                    onClick={() => {
                      updateArInvoice(inv.id, {
                        status: "PAID",
                        paidDate: new Date().toISOString().slice(0, 10),
                        receivedAmount: inv.totalAmount,
                ...
                {(inv.status === "DRAFT" || inv.status === "SENT") && (
                  <Button
                    variant="destructive"
                    onClick={() => {
                      updateArInvoice(inv.id, { status: "VOID" });
                ...
                {(inv.status === "PAID" || inv.status === "PARTIALLY_PAID") && (
                  ...
                  >
                    <Receipt className="size-4 mr-1.5" /> Download receipt
                  </Button>
                )}
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <Row label="Client" v={inv.client} />
            <Row label="Client email" v={inv.clientEmail ?? "—"} />
            <Row label="Counselor" v={inv.counselor} />
            <Row label="Entity" v={inv.entity} />
            <Row label="Branch" v={inv.branch} />
            ...
            <Row label="Issued" v={inv.invoiceDate} />
            <Row label="Due date" v={inv.dueDate} />
          ...
          <CardContent>
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <div className="text-lg font-medium tabular-nums">{fmtMoney(inv.subtotal, inv.currency)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Tax ({inv.taxCode})</div>
                <div className="text-lg font-medium tabular-nums">{fmtMoney(inv.taxAmount, inv.currency)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Total</div>
                <div className="text-2xl font-bold tabular-nums">{fmtMoney(inv.totalAmount, inv.currency)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Outstanding</div>
                <div className="text-2xl font-bold tabular-nums text-destructive">
                  {fmtMoney(inv.outstandingBalance, inv.currency)}
                </div>
              </div>
            </div>
            {inv.installmentPlan && (
              <div className="mt-3 text-xs text-muted-foreground">
                Installment plan · {inv.installmentsPaid}/{inv.totalInstallments} paid
              </div>
            )}
            {inv.status === "PAID" && (
              <div className="mt-3 text-xs text-green-700 dark:text-green-400">
                Received {inv.paidDate} · Ref {inv.paymentReference ?? "—"}
              </div>
            )}
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap gap-2 text-xs">
            ...
            {!bank && !inv.linkedJournalId && !inv.linkedDocumentId && (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {["SENT", "PARTIALLY_PAID", "PAID", "OVERDUE"].includes(inv.status) && (
              <Timeline color="bg-muted-foreground" label="Invoice sent to client" ts={inv.invoiceDate} />
            )}
            {inv.viewedAt && <Timeline color="bg-muted-foreground" label="Client viewed invoice" ts={inv.viewedAt} />}
            {inv.status === "PARTIALLY_PAID" && (
              <Timeline
                color="bg-amber-500"
                label={`Partial payment received — ${fmtMoney(inv.receivedAmount, inv.currency)}`}
                ts={inv.paidDate ?? inv.invoiceDate}
              />
            )}
            {inv.status === "PAID" && (
              <Timeline
                color="bg-green-500"
                label={`Payment received — ${inv.paymentReference ?? ""} · ${fmtMoney(inv.totalAmount, inv.currency)}`}
                ts={inv.paidDate ?? ""}
              />
            )}
            {inv.status === "OVERDUE" && (
              <Timeline
                ...
                label={`Marked overdue — ${inv.daysOverdue ?? "?"} days past due`}
                ts={inv.dueDate}
              />
            )}
            {inv.status === "VOID" && (
              <Timeline
                color="bg-destructive"
                label={`Voided${inv.notes ? ` — ${inv.notes}` : ""}`}
                ts={inv.invoiceDate}
              />
            )}
          </CardContent>
