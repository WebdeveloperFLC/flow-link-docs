...
export default function AccountingARPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [tab, entityFilter, branchFilter, currencyFilter, serviceFilter, statusFilter, dateFrom, dateTo, search]);
  ...
  const dueThisWeek = sumBy(
    (i) =>
      ["SENT", "PARTIALLY_PAID"].includes(i.status) && daysFromToday(i.dueDate) >= 0 && daysFromToday(i.dueDate) <= 7,
  );
  const collectedThisMonth = sumBy(
    (i) => i.status === "PAID" && (i.paidDate?.startsWith("2024-10") || i.paidDate?.startsWith("2024-11") || false),
    "totalAmount",
  );
  ...
  function exportCSV() {
    const rows = [
      ...
      ...filtered.map((i) => [
        i.invoiceNumber,
        i.client,
        SERVICE_TYPE_LABELS[i.serviceType],
        i.counselor,
        i.entity,
        i.branch,
        i.invoiceDate,
    ...
    a.href = url;
    a.download = `ar-invoices-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="p-6 max-w-[1400px] mx-auto">
        <AccountingPageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => navigate("/accounting/ar/new")}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <AccountingKPICard
            label="Overdue"
            value={overdueAmt}
            delta={`${overdueCount} invoices overdue`}
            deltaDirection="down"
            icon={AlertCircle}
          />
        ...
        <div className="flex flex-wrap gap-3 mb-4 items-end">
          ...
          <SelectFilter
            ...
            options={["DRAFT", "SENT", "PARTIALLY_PAID", "PAID", "OVERDUE", "VOID"]}
            width="w-[150px]"
          />
        ...
        {loading ? (
          ...
          <AccountingEmptyState
            ...
            description="Try adjusting your filters or create your first customer invoice."
            action={
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {i.entity}
                        {i.branch && i.branch !== i.entity ? ` · ${i.branch}` : ""}
                      </div>
                    </td>
                    <td className="px-3 py-3 max-w-[280px]">
                      <div className="text-[11px] inline-block bg-muted px-1.5 py-0.5 rounded">
                        {SERVICE_TYPE_LABELS[i.serviceType]}
                      </div>
                      <div className="truncate text-xs text-muted-foreground mt-0.5" title={i.description}>
                        {i.description}
                      </div>
                    </td>
                    <td className="px-3 py-3 tabular-nums text-xs">{i.invoiceDate}</td>
                    <td className="px-3 py-3 text-xs">
                      <div className={cn("tabular-nums", i.status === "OVERDUE" && "text-destructive font-medium")}>
                        {i.dueDate}
                      </div>
                      {i.status === "OVERDUE" && i.daysOverdue && (
                        <div className="text-[10px] text-destructive">{i.daysOverdue} days overdue</div>
                      )}
                    </td>
                    <td className="px-3 py-3 text-right tabular-nums">
                      <div className="font-medium">{fmtMoney(i.totalAmount, i.currency)}</div>
                      <div className="text-[10px] text-muted-foreground">
                        {i.currency} · {i.taxCode}
                      </div>
                    </td>
                    <td className="px-3 py-3 text-right tabular-nums">
                      <div
                        className={cn(
                          "font-medium",
                          i.outstandingBalance > 0 && i.status === "OVERDUE" && "text-destructive",
                        )}
                      >
                        {fmtMoney(i.outstandingBalance, i.currency)}
                      </div>
                      {i.installmentPlan && (
                        <div className="text-[10px] text-muted-foreground">
                          {i.installmentsPaid}/{i.totalInstallments} paid
                        </div>
                      )}
                    </td>
                    <td className="px-3 py-3">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {!i.linkedBankAccountId && !i.linkedJournalId && (
                          <span className="text-muted-foreground text-xs">—</span>
                        )}
                      </div>
                    </td>
                    <td className="px-3 py-3">
                      <AccountingStatusBadge status={i.status} />
                    </td>
                    <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => navigate(`/accounting/ar/${i.id}`)}>
                            View details
                          </DropdownMenuItem>
                          ...
                          {i.status === "DRAFT" && (
                            <DropdownMenuItem
                              onClick={() => {
                                updateInvoice(i.id, { status: "SENT" });
                          ...
                          <DropdownMenuItem
                            onClick={() => {
                              ...
                              toast.info("Fill in journal details manually");
                            }}
                          >
                            Create journal entry
                          </DropdownMenuItem>
                          {i.clientId ? (
                            <DropdownMenuItem onClick={() => navigate(`/accounting/clients/${i.clientId}`)}>
                              View client ledger
                            </DropdownMenuItem>
                          ) : (
                          ...
                          {(i.status === "DRAFT" || i.status === "SENT") && (
                            <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-destructive" onClick={() => setVoidDialog(i)}>
                                Void
                              </DropdownMenuItem>
                            </>
                          )}
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="text-destructive" onClick={() => setDeleteDialog(i)}>
                            Delete
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
          ...
          <RecordReceiptDialog
            ...
            onConfirm={(patch) => {
              ...
              updateInvoice(payDialog.id, {
                ...
                linkedBankAccountId: patch.linkedBankAccountId ?? payDialog.linkedBankAccountId,
              });
        ...
        <AlertDialog open={!!voidDialog} onOpenChange={(o) => !o && setVoidDialog(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {voidDialog && `${voidDialog.invoiceNumber} — ${voidDialog.client}. This cannot be undone.`}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  if (voidDialog) {
                    updateInvoice(voidDialog.id, { status: "VOID" });
        ...
        <AlertDialog open={!!deleteDialog} onOpenChange={(o) => !o && setDeleteDialog(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {deleteDialog &&
                  `${deleteDialog.invoiceNumber} — ${deleteDialog.client}. This permanently removes the record.`}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction
                onClick={() => {
                  if (deleteDialog) {
                    deleteArInvoice(deleteDialog.id);
...
function AgingAnalysis({ invoices }: { invoices: CustomerInvoice[] }) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        <CardTitle className="text-base">Receivables aging analysis</CardTitle>
      </CardHeader>
      <CardContent>
    ...
    <AlertDialog open onOpenChange={(o) => !o && onClose()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {invoice.client} — outstanding {fmtMoney(invoice.outstandingBalance, invoice.currency)}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="grid gap-3 py-2">
          ...
          <div className="grid gap-1.5">
            ...
            <Select value={bankId} onValueChange={setBankId}>
              <SelectTrigger>
                <SelectValue placeholder={banks.length ? "Select bank" : `No ${invoice.currency} bank accounts`} />
              </SelectTrigger>
              <SelectContent>
                {banks.map((b) => (
          ...
          <AlertDialogAction
            onClick={() =>
              onConfirm({
                ...
                paidDate,
                paymentReference,
                paymentMethod: paymentMethod.toUpperCase().replace(/ /g, "_") as CustomerInvoice["paymentMethod"],
