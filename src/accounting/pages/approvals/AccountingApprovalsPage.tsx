...
import { useApBills, updateApBill } from "../../stores/apBillsStore";
...
const STATUS_PILL: Record<ApprovalStatus, { label: string; cls: string }> = {
  ...
  AUDITOR1_REVIEW: {
    label: "Auditor 1 review",
    cls: "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300",
  },
  AUDITOR2_REVIEW: {
    label: "Auditor 2 review",
    cls: "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300",
  },
  FINAL_REVIEW: {
    label: "Final review",
...
export default function AccountingApprovalsPage() {
  ...
  const apBills = useApBills();
  const [mockRequests, setMockRequests] = useState<PaymentRequest[]>(MOCK_APPROVALS);
    ...
    (): PaymentRequest[] =>
      apBills
        ...
        .map(
          (b): PaymentRequest => ({
            id: `ap-${b.id}`,
            requestNumber: b.billNumber,
            entity: b.entity ?? "—",
            payeeName: b.vendor,
            amount: b.totalAmount,
            currency: (["CAD", "USD", "INR"].includes(b.currency) ? b.currency : "CAD") as "CAD" | "USD" | "INR",
            description: b.description || `AP Bill — ${b.vendor}`,
            dueDate: b.dueDate,
            status: "SUBMITTED" as ApprovalStatus,
            currentStep: 1,
            submittedBy: b.createdBy || "Staff",
            submittedAt: b.billDate ? `${b.billDate}T00:00:00Z` : new Date().toISOString(),
            daysPending: Math.max(
              0,
              Math.floor((Date.now() - new Date(b.billDate || Date.now()).getTime()) / 86400000),
            ),
            steps: [
              {
                stepNumber: 1,
                name: "Submitted",
                role: "ACCOUNTANT",
                assignee: b.createdBy || "Staff",
                status: "approved" as const,
                completedAt: b.billDate ? `${b.billDate}T00:00:00Z` : new Date().toISOString(),
              },
              {
                stepNumber: 2,
                name: "Manager review",
  ...
  const requests = useMemo(() => [...liveRequests, ...mockRequests], [liveRequests, mockRequests]);
  ...
  const stats = useMemo(() => {
    ...
    const me = requests
      .filter((r) => r.currentStep === 2 || r.currentStep === 3)
      .filter((r) => !TERMINAL.includes(r.status)).length;
  ...
  const filteredByTab = useMemo(() => {
    switch (tab) {
      case "pending":
        return requests.filter((r) => !TERMINAL.includes(r.status));
      case "me":
        return requests.filter((r) => r.status === "AUDITOR1_REVIEW" || r.status === "AUDITOR2_REVIEW");
      case "completed":
        return requests.filter((r) => TERMINAL.includes(r.status));
      default:
        return requests;
  ...
  const advance = (target: PaymentRequest) => {
    ...
    if (!next) return;
    if (target.id.startsWith("ap-")) {
      const billId = target.id.replace("ap-", "");
      if (next === "APPROVED") {
        updateApBill(billId, { status: "APPROVED" });
        toast.success(`${target.requestNumber} approved — ready for payment`);
      } else {
        toast.success(`${target.requestNumber} forwarded to ${STATUS_PILL[next].label}`);
      }
      return;
    }
    setMockRequests((prev) =>
      prev.map((r) => {
        ...
        const newSteps = r.steps.map((s) =>
          s.stepNumber === r.currentStep
            ? {
                ...s,
                status: "APPROVED" as const,
                actionAt: new Date().toISOString(),
                ipAddress: "10.0.14.22",
                deviceHint: "MacBook Pro · Chrome",
                comments: s.comments ?? "Reviewed and approved.",
              }
            : s,
        );
  ...
  const reject = (target: PaymentRequest, reason: string) => {
    if (target.id.startsWith("ap-")) {
      const billId = target.id.replace("ap-", "");
      updateApBill(billId, { status: "VOID" });
      toast.success(`${target.requestNumber} rejected — bill voided`);
      return;
    }
    setMockRequests((prev) =>
      prev.map((r) => {
  ...
  const cancel = (target: PaymentRequest) => {
    if (target.id.startsWith("ap-")) {
      const billId = target.id.replace("ap-", "");
      updateApBill(billId, { status: "VOID" });
      toast.success("Request cancelled — bill voided");
      return;
    }
    setMockRequests((prev) => prev.map((r) => (r.id === target.id ? { ...r, status: "CANCELLED" as const } : r)));
                ...
                <td className="px-2 py-2.5" onClick={(e) => e.stopPropagation()}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => navigate(`/accounting/approvals/${r.id}`)}>
                        View details
                      </DropdownMenuItem>
                      {canAct && (
                        <>
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="pending">
              Pending ({requests.filter((r) => !TERMINAL.includes(r.status)).length})
            </TabsTrigger>
            <TabsTrigger value="me">
              Awaiting me (
              {requests.filter((r) => r.status === "AUDITOR1_REVIEW" || r.status === "AUDITOR2_REVIEW").length})
            </TabsTrigger>
            <TabsTrigger value="completed">
              Completed ({requests.filter((r) => TERMINAL.includes(r.status)).length})
            </TabsTrigger>
          </TabsList>
      ...
      <AlertDialog open={!!approveTarget} onOpenChange={(o) => !o && setApproveTarget(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (approveTarget) advance(approveTarget);
