...
import { matchesExpenseCategory } from "../../lib/coaCategoryMap";
...
export default function AccountingNewBillPage() {
  ...
  const [expenseCoaId, setExpenseCoaId] = useState("");
  const [apCoaId, setApCoaId] = useState("");
  ...
  const eligibleBanks = bankAccounts.filter(
    (b) => b.status === "ACTIVE" && b.entityId === entityId && b.currency === currency && b.isDefaultPayment !== false, // exclude savings/holding accounts
  );
  ...
  const eligibleExpenseAccounts = accounts.filter(
    (a) => coaScope(a) && (a.groupCode === "EXPENSE" || a.groupCode === "ASSET") && matchesExpenseCategory(a, category),
  );

  const eligibleApAccounts = accounts.filter((a) => coaScope(a) && a.groupCode === "LIABILITY" && a.typeCode === "AP");
  ...
  function buildPayload(status: VendorBill["status"]): Omit<VendorBill, "id"> {
    const cc = (["CA", "US", "IN", "AE"] as const).includes(country as never)
      ? (country as VendorBill["branchCountry"])
      : "OTHER";
    const cur = (["CAD", "USD", "INR", "AED", "GBP", "AUD", "EUR"] as const).includes(currency as never)
      ? (currency as VendorBill["currency"])
      : "CAD";
    const pm = (["BANK_TRANSFER", "CHEQUE", "CASH", "CREDIT_CARD", "UPI", "WIRE", "OTHER"] as const).includes(
      payMethod as never,
    )
      ? (payMethod as VendorBill["paymentMethod"])
      : undefined;
  ...
  function save(asDraft: boolean) {
    if (asDraft) {
      if (!vendor || !billNumber || !entityId) {
        toast.error("Vendor, bill number and entity are required to save a draft");
        return;
      }
    ...
    if (!r.success) {
      toast.error(r.error.errors[0]?.message ?? "Please complete required fields");
      return;
    }
    if (dueDate < billDate) {
      toast.error("Due date must be on or after bill date");
      return;
    }
    if (!expenseCoaId) {
      toast.error("Pick an expense/asset account from the chart of accounts");
      return;
    }
    if (!apCoaId) {
      toast.error("No AP control account available for this entity / currency — add one in Chart of accounts");
      return;
    }
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-5">
        <div className="sticky top-0 bg-background z-10 -mx-6 px-6 py-3 border-b border-border">
          <AccountingPageHeader
            title="New bill"
            subtitle="AP — Bills / New bill"
            actions={
              <>
                <Button variant="ghost" onClick={() => navigate("/accounting/ap")}>
                  Discard
                </Button>
          ...
          <CardContent className="grid grid-cols-2 gap-3">
            ...
            <Field label="Expense category">
              <FreeCombobox
                ...
                options={[...Object.values(EXPENSE_CATEGORY_LABELS), "Other — type your own"]}
              />
            </Field>
            <Field label="Department">
              <FreeCombobox
                ...
                options={[...DEPARTMENTS, "Other — type your own"]}
              />
            </Field>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Entity & branch</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 gap-3">
            <Field label="Entity *">
              <Select
            ...
            <Field label="Branch">
              <Select
                ...
                disabled={!entityId || branches.length === 0}
              >
                <SelectTrigger>
                  <SelectValue placeholder={branches.length ? "Select branch" : "No branches"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field label="Branch country">
              <DynamicSelect listKey="countries" value={country} onValueChange={setCountry} />
            </Field>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-3">
            <Field label="Bill number *">
              <Input value={billNumber} onChange={(e) => setBillNumber(e.target.value)} placeholder="BILL-2024-001" />
            </Field>
            ...
            <Field label="Tax code">
              <DynamicSelect listKey="tax_codes" value={taxCode} onValueChange={applyTax} />
            </Field>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-3">
            <Field label="Expense / asset account (Dr) *">
              <Select
                value={expenseCoaId || "__none__"}
                onValueChange={(v) => setExpenseCoaId(v === "__none__" ? "" : v)}
                disabled={!entityId || !category}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={
                ...
                <SelectContent>
                  {!entityId ? (
                    ...
                    <SelectItem value="__none__" disabled>
                      No COA account mapped for this category — add one in Chart of Accounts.
                    </SelectItem>
                  ) : (
            ...
            <Field label="AP control account (Cr)">
              <Select
                value={apCoaId || "__none__"}
                onValueChange={(v) => setApCoaId(v === "__none__" ? "" : v)}
                disabled={!entityId}
              >
                <SelectTrigger>
                  <SelectValue placeholder={entityId ? "Auto-selected" : "Select an entity first"} />
                </SelectTrigger>
                <SelectContent>
                  {!entityId ? (
                    <SelectItem value="__none__" disabled>
                    ...
                    <SelectItem value="__none__" disabled>
                      No {currency} AP account for this entity — add one in Chart of accounts
                    </SelectItem>
                  ) : (
                ...
                <SelectContent>
                  ...
                  {!entityId ? (
                    ...
                    <SelectItem value="__empty__" disabled>
                      No {currency} bank accounts for this entity — add one in Bank accounts
                    </SelectItem>
                  ) : (
                    eligibleBanks.map((b) => (
                      <SelectItem key={b.id} value={b.id}>
            ...
            <Field label="Payment method">
              <DynamicSelect listKey="payment_methods" value={payMethod} onValueChange={setPayMethod} />
            </Field>
            <Field label="Tags">
              <div className="flex flex-wrap gap-1.5 items-center min-h-9 px-2 py-1 border border-input rounded-md">
                {tags.map((t) => (
                  <span key={t} className="bg-accent text-xs px-2 py-0.5 rounded inline-flex items-center gap-1">
                    {t}
