...
function hasFullLanguage(a: Record<string, any>): boolean {
  ...
  return ["english_listening", "english_reading", "english_writing", "english_speaking"].every((k) => {
    const n = Number(a[k]);
...
export default function AssessmentRun() {
  ...
  useEffect(() => {
    ...
    if (isCanada(country) && !hasFullLanguage(answers)) {
      setCrs(null);
      return;
    }
    ...
    crsTimer.current = window.setTimeout(async () => {
      try {
        ...
        if (!error && !(data as any)?.error && isMounted.current) setCrs(data);
      } catch {
        /* ignore */
      }
    }, 350);
  ...
  useEffect(() => {
    ...
    if (current) return;
    const inferred: "citizen" | "pr_holder" | null = isCanadianCitizen
      ? "citizen"
      : familyStatus === "pr_holder"
        ? "pr_holder"
            ...
            <>
              <div className="flc-card p-6 space-y-3">
                ...
                <div className="grid sm:grid-cols-2 gap-2 text-sm">
                  ...
                  <SummaryChip
                    ok={Number(answers.canadian_work_experience ?? 0) >= 1}
                    label="CEC potential (1+ yr Canadian work)"
                  />
              ...
              {fsw && (
                <div className="flc-card p-6 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-x-6 text-sm pt-2">
                    ...
                    <RowItem
                      label={`Experience (max ${fsw.sections.experience.max})`}
                      v={fsw.sections.experience.total}
                    />
                    <RowItem label={`Age (max ${fsw.sections.age.max})`} v={fsw.sections.age.total} />
                    <RowItem
                      label={`Arranged employment (max ${fsw.sections.arranged_employment.max})`}
                      v={fsw.sections.arranged_employment.total}
                    />
                    <RowItem
                      label={`Adaptability (max ${fsw.sections.adaptability.max})`}
                      v={fsw.sections.adaptability.total}
                    />
                  </div>
                  {fsw.notes?.length > 0 && (
                    <ul className="text-[11px] text-[hsl(220_14%_45%)] list-disc pl-5 pt-2 space-y-0.5">
                      {fsw.notes.map((n: string, i: number) => (
                        <li key={i}>{n}</li>
                      ))}
                    </ul>
                  )}
                  ...
                  >
                    <span
                      className={`size-5 rounded-full flex items-center justify-center text-[10px] font-semibold ${
                        active
                          ? "bg-white text-[hsl(220_18%_11%)]"
                          : done
                            ? "bg-[hsl(8_75%_60%)] text-white"
                            : "border border-[hsl(30_12%_82%)]"
                      }`}
                    >
                      {done ? "✓" : i + 1}
                    </span>
                  ...
                  currentQuestions.map((q) => (
                    <div key={q.id} className="space-y-2">
                      <label className="text-sm font-medium text-[hsl(220_18%_11%)]">
                        {q.label}
                        {q.required && <span className="text-[hsl(8_75%_60%)] ml-0.5">*</span>}
                      </label>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 rounded-full border border-[hsl(30_12%_82%)] text-sm font-medium hover:bg-[hsl(36_20%_94%)]"
              >
                {downloading ? (
                  <Loader2 className="size-3.5 inline mr-1.5 animate-spin" />
                ) : (
              ...
              >
                {saving ? (
                  ...
                  <Save className="size-3.5 inline mr-1.5" />
                )}
                Save
              </button>
              {isFamilyFlow ? (
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full border text-sm ${
              v === val
                ...
                : "border-[hsl(30_12%_82%)] bg-white"
            }`}
          >
            {String(lbl)[0].toUpperCase() + String(lbl).slice(1)}
          </button>
        ))}
      </div>
    );
        ...
        {opts.map((o) => (
          ...
          >
            <span className="inline-flex items-center gap-2">
              <span
                className={`size-3 rounded-full border ${v === o.value ? "bg-white border-white" : "border-[hsl(30_12%_70%)]"}`}
              />
              {o.label}
            </span>
          </button>
