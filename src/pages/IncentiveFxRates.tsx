...
export default function IncentiveFxRates() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from("fx_rates")
      .select("id, currency, period_key, base_rate_to_inr, rate_to_inr, buffer_fixed, buffer_pct, source")
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Effective rate = base + buffer (default +2). Used for billing and incentive settlement in V1.
            </p>
          </div>
        </div>
