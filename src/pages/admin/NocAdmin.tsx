      ...
      <AppLayout>
        ...
        <div className="p-8">
          <Card className="p-12 text-center text-sm text-muted-foreground">
            Only administrators can manage the NOC database.
          </Card>
        </div>
      </AppLayout>
    );
  ...
  return (
    <Card className="mt-4">
      <div className="flex items-center gap-2 p-3 border-b">
        ...
        >
          <Plus className="size-4 mr-1.5" /> Add occupation
        </Button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="text-left px-3 py-2">TEER</th>
            ...
            {rows.map((r) => (
              <tr key={r.noc_code} className="border-b hover:bg-muted/30">
                ...
                <td className="px-3 py-2 font-medium">
                  {r.title}
                  <div className="text-xs text-muted-foreground">{r.broad_category}</div>
                </td>
                <td className="px-3 py-2">{r.teer}</td>
                <td className="px-3 py-2 text-xs">
                  {(r.categories ?? []).join(", ") || <span className="text-muted-foreground">—</span>}
                </td>
                ...
                <td className="px-3 py-2 text-right">
                  ...
                  <Button size="icon" variant="ghost" className="size-7 text-destructive" onClick={() => onDelete(r)}>
...
function OccDialog({
  ...
  useEffect(() => {
    if (open) {
      setCode(item?.noc_code ?? "");
      setTitle(item?.title ?? "");
      setTeer(item?.teer ?? 1);
      setBroad(item?.broad_category ?? "");
      setKeywords((item?.keywords ?? []).join(", "));
      setCats(item?.categories ?? []);
    }
  }, [open, item?.noc_code]);
  // Depend on noc_code (stable identity) instead of the whole `item` object,
  // so background re-fetches don't overwrite the user's in-progress edits.
  const save = async () => {
    if (!code.trim() || !title.trim()) {
      toast.error("NOC code and title required");
      return;
    }
    setBusy(true);
    try {
      const payload = {
        ...
        title: title.trim(),
        teer,
        broad_category: broad.trim() || null,
        keywords: keywords
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
      ...
      if (cats.length) {
        await supabase
          ...
          .insert(cats.map((c) => ({ noc_code: payload.noc_code, category: c })));
      }
      toast.success("Saved");
      onOpenChange(false);
      onSaved();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{item ? "Edit occupation" : "Add occupation"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>NOC code *</Label>
              <Input value={code} onChange={(e) => setCode(e.target.value)} disabled={!!item} placeholder="21231" />
            </div>
            <div>
              <Label>TEER *</Label>
              <select
                ...
                value={teer}
                onChange={(e) => setTeer(Number(e.target.value))}
              >
                {[0, 1, 2, 3, 4, 5].map((t) => (
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Natural and applied sciences"
            />
          </div>
          <div>
              ...
              {CATEGORIES.map((c) => {
                ...
                return (
                  <button
                    type="button"
                    key={c}
                    onClick={() => setCats((a) => (on ? a.filter((x) => x !== c) : [...a, c]))}
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
          <tr>
            ...
            <th className="text-left px-3 py-2">Allowed TEERs</th>
            <th className="text-left px-3 py-2">Foreign yrs</th>
            <th className="text-left px-3 py-2">Canadian yrs</th>
            <th className="text-left px-3 py-2">Min CLB</th>
            <th className="text-left px-3 py-2">Job offer</th>
            <th className="text-left px-3 py-2">Active</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b">
              ...
              <td className="px-3 py-2">
                <Input
                  className="h-8 w-32"
                  defaultValue={(r.allowed_teers ?? []).join(",")}
                  onBlur={(e) =>
                    update(r.id, {
                      allowed_teers: e.target.value
                        .split(",")
                        .map((s) => Number(s.trim()))
                        .filter((n) => !isNaN(n)),
                    })
              ...
              <td className="px-3 py-2">
                <Input
                  type="number"
                  className="h-8 w-20"
                  defaultValue={r.min_foreign_experience_years ?? ""}
                  onBlur={(e) =>
                    update(r.id, {
                      min_foreign_experience_years: e.target.value === "" ? null : Number(e.target.value),
                    })
                  }
                />
              </td>
              <td className="px-3 py-2">
                <Input
                  type="number"
                  className="h-8 w-20"
                  defaultValue={r.min_canadian_experience_years ?? ""}
                  onBlur={(e) =>
                    update(r.id, {
                      min_canadian_experience_years: e.target.value === "" ? null : Number(e.target.value),
                    })
                  }
                />
              </td>
              <td className="px-3 py-2">
                <Input
                  type="number"
                  className="h-8 w-20"
                  defaultValue={r.min_clb ?? ""}
                  onBlur={(e) => update(r.id, { min_clb: e.target.value === "" ? null : Number(e.target.value) })}
                />
              </td>
              <td className="px-3 py-2">
                <Switch
                  checked={r.requires_job_offer}
                  onCheckedChange={(v) => update(r.id, { requires_job_offer: v })}
                />
              </td>
              <td className="px-3 py-2">
...
function ProvincialTab() {
  ...
  const add = async () => {
    if (!form.province_code || !form.province_name || !form.stream_name) {
      toast.error("Province + stream required");
      return;
    }
    const payload: any = {
  ...
  const del = async (id: string) => {
    ...
    await supabase.from("provincial_noc_targets").delete().eq("id", id);
    load();
  };
  return (
    <Card className="mt-4">
      <div className="p-3 grid grid-cols-2 md:grid-cols-7 gap-2 border-b">
        ...
        <Input
          placeholder="NOC (opt)"
          value={form.noc_code}
          onChange={(e) => setForm({ ...form, noc_code: e.target.value })}
        />
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b">
                ...
                <td className="px-3 py-2">
                  {r.stream_name}
                  <div className="text-xs text-muted-foreground">{r.province_name}</div>
                </td>
                <td className="px-3 py-2 font-mono text-xs">{r.noc_code ?? "—"}</td>
                <td className="px-3 py-2">{r.teer ?? "—"}</td>
                ...
                <td className="px-3 py-2 text-right">
                  <Button size="icon" variant="ghost" className="size-7 text-destructive" onClick={() => del(r.id)}>
                    <Trash2 className="size-3.5" />
                  </Button>
                </td>
              </tr>
