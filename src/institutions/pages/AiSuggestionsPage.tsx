...
export default function AiSuggestionsPage() {
  ...
  const setStatus = async (ids: string[], status: string) => {
    if (!canEdit) return toast.error("View-only access — cannot update suggestions");
  ...
  return (
    <AppLayout>
      ...
      <div className="p-6 space-y-4">
        {!canEdit && <ViewOnlyNotice label="Institutions (AI Suggestions)" />}
                ...
                <Card key={s.id} className="p-4 space-y-2 border-l-4 border-l-primary/40">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      {canEdit && <Checkbox checked={selected.has(s.id)} onCheckedChange={() => toggle(s.id)} />}
