...
export function AttendanceRegisterView() {
  ...
  const exportRegister = (fmt: "CSV" | "Excel") => {
    downloadAttendanceRegister(registerRows, from, to, fmt);
    fire(`${fmt} exported`);
  };

  const exportPdf = () => {
    printAttendanceRegister(registerRows, from, to);
    ...
    <div className="page-grid">
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="card-h" style={{ padding: "14px 16px" }}>
          <div>
            <h3 style={{ fontSize: 15, marginBottom: 2 }}>All-employee register</h3>
            <p className="muted" style={{ fontSize: 12, margin: 0 }}>
              {from} to {to} · {registerRows.length} records · {filteredEmployees.length} employees
            </p>
          </div>
          <div className="row-flex" style={{ gap: 8 }}>
            <Link to="/hr/reports/attendance" className="btn btn-sm">
              Summary report →
            </Link>
            {canExport && (
              <>
                <button type="button" className="btn btn-sm" onClick={() => exportRegister("CSV")}>
                  CSV
                </button>
                <button type="button" className="btn btn-sm" onClick={() => exportRegister("Excel")}>
                  Excel
                </button>
        ...
        {loading ? (
          <div className="empty">Loading attendance register…</div>
        ) : filteredEmployees.length === 0 ? (
          <div className="empty empty-sm">No employees match your filters.</div>
        ) : (
          <AttendanceRegisterTable rows={registerRows} profileSearch={profileSearch} />
        )}
      </div>
    </div>
  );
