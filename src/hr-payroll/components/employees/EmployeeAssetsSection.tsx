...
function updateAsset(
  ...
  if (patch.asset_type && !assetSupportsMac(patch.asset_type)) current.mac_address = "";
  if (patch.asset_type && !assetSupportsImei(patch.asset_type)) current.imei_number = "";
...
export function EmployeeAssetsSection({ assets, onChange, staffOptions, errors }: Props) {
  return (
    <div className="grid" style={{ gap: 16 }}>
      <div className="row-flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button
          ...
          onClick={() => onChange([...assets, newAssetDraft()])}
        >
          + Add Asset
        </button>
      </div>
      ...
      {assets.length === 0 ? (
        ...
        assets.map((a, i) => {
          ...
          const showTypeOther = a.asset_type === "Other";
          const showAccOther = a.accessories.includes("Other");
          const title = [a.asset_type, a.asset_name, a.serial_number].filter(Boolean).join(" · ") || `Asset ${i + 1}`;

          return (
            <div
              key={a._key}
              className="card"
              style={{ background: "var(--paper)", padding: 16, display: "grid", gap: 12 }}
            >
              ...
              <div className="grid g2" style={{ gap: "0 16px" }}>
                ...
                {showTypeOther && (
                  <label className="fld">
                    ...
                    />
                    {errors[`${p}_type_other`] && <div className="errmsg">{errors[`${p}_type_other`]}</div>}
                  </label>
                )}
                <label className="fld">
                ...
                {assetSupportsMac(a.asset_type) && (
                  <label className="fld">
                    <span className="l">MAC Address</span>
                    <input
                      className={`input${errors[`${p}_mac`] ? " err" : ""}`}
                      value={a.mac_address}
                      onChange={(e) => onChange(updateAsset(assets, i, { mac_address: e.target.value }))}
                    />
                    {errors[`${p}_mac`] && <div className="errmsg">{errors[`${p}_mac`]}</div>}
                  </label>
                )}
                {assetSupportsImei(a.asset_type) && (
                  <label className="fld">
                    <span className="l">IMEI Number</span>
                    <input
                      className={`input${errors[`${p}_imei`] ? " err" : ""}`}
                      value={a.imei_number}
                      onChange={(e) => onChange(updateAsset(assets, i, { imei_number: e.target.value }))}
                    />
                    {errors[`${p}_imei`] && <div className="errmsg">{errors[`${p}_imei`]}</div>}
                  </label>
                )}
              </div>
              <label className="fld">
              ...
              <div className="grid g2" style={{ gap: "0 16px" }}>
                ...
                <label className="fld">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onChange(updateAsset(assets, i, { asset_status: e.target.value as EmployeeAssetDraft["asset_status"] }))
                    }
                  >
                ...
                <>
                  ...
                  <div className="grid g2" style={{ gap: "0 16px" }}>
                    <label className="fld">
                      ...
                      />
                      {errors[`${p}_return_date`] && <div className="errmsg">{errors[`${p}_return_date`]}</div>}
                    </label>
                    <label className="fld">
                      ...
                      {errors[`${p}_collected_by`] && <div className="errmsg">{errors[`${p}_collected_by`]}</div>}
                    </label>
                    <label className="fld">
