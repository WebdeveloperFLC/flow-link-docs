...
function collectRefSyncEntries(state: ProfileEditState): { ref_key: string; linked_documents: { document_id: string; slot: string; label: string; linked_at?: string | null }[] }[] {
  ...
  for (const e of state.tests.english) {
    entries.push({
      ref_key: englishTestRefKey(e.test_type),
  ...
  for (const a of state.tests.aptitude) {
    entries.push({
      ref_key: aptitudeTestRefKey(a.test_type),
  ...
  for (const l of state.tests.language) {
    entries.push({
      ref_key: languageTestRefKey(l.language),
...
export async function profileSave(
  ...
  if (sections.has("tests")) {
    Object.assign(
      clientPatch,
      englishEntriesToClientFields(state.tests.active_english_test, state.tests.english),
    );
    clientPatch.other_tests = aptitudeEntriesToOtherTests(state.tests.aptitude, state.tests.language);
