import { getSlotsForScope } from "@/lib/profile/profileDocumentSlots";
import type {
...
function countTests(vm: ProfileViewModel): ProfileCompletionSection {
  ...
  const activeEnglish = vm.tests.english.find((e) => e.test_type === vm.tests.active_english_test);
...
function experienceRecordScore(e: ProfileExperienceRecord): number {
  ...
  const filled = keys.filter((k) => {
    if (k === "currently_working") return false;
    return filledStr(e[k] as string | null);
  }).length;
  return filled / keys.length;
}
...
function missingRequiredDocuments(vm: ProfileViewModel): string[] {
  ...
  const testSlots = getSlotsForScope("tests").map((s) => s.id);

  for (const e of vm.tests.english) {
    if (e.status === "taken" && e.linked_documents.length === 0) {
      missing.push(`${e.test_type}: score report or TRF`);
    }
  }
