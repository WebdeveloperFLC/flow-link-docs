...
/** Score document type against query; 0 = no match. */
export function scoreDocumentTypeMatch(item: MasterItem, query: string): number {
  const q = norm(query);
  if (!q) return 1;
  ...
  if (label === q || code === q) return 100;
  if (label.startsWith(q) || code.startsWith(q)) return 90;
  if (label.includes(q) || code.includes(q)) return 80;

  for (const alias of metadataAliases(item)) {
    const a = norm(alias);
    if (a.includes(q) || q.includes(a)) return 75;
  }

  for (const [term, codes] of Object.entries(SEARCH_ALIASES)) {
    if (!term.includes(q) && !q.includes(term)) continue;
    if (codes.includes(item.code)) return 85;
    if (codes.some((c) => label.includes(norm(c.replace(/_/g, " "))))) return 85;
  }

  if (isChecklistAlias(q, item.label) || isChecklistAlias(item.label, q)) return 70;

  const tokens = q.split(/\s+/).filter(Boolean);
  if (tokens.length > 1 && tokens.every((t) => label.includes(t) || code.includes(t))) return 65;
...
export function filterDocumentTypesForSearch(
  ...
  return items
    ...
    .map((item) => ({ item, score: scoreDocumentTypeMatch(item, query) }))
