...
function mapStudentApplication(row: Record<string, unknown>): StudentApplicationRecord {
  ...
  return {
    ...
    applicationFee: row.application_fee != null ? Number(row.application_fee) : null,
    applicationFeeCurrency: (row.application_fee_currency as string | null) ?? null,
    partnershipRouteId: (row.partnership_route_id as string | null) ?? null,
    feeSnapshotJsonb: (row.fee_snapshot_jsonb as Record<string, unknown>[] | null) ?? null,
...
export async function upsertStudentApplication(payload: UpsertStudentApplicationPayload): Promise<string> {
  const { data, error } = await supabase.rpc("fn_upsert_client_qualification" as never, {
    p_payload: {
      ...
      application_fee_currency: payload.applicationFeeCurrency ?? null,
      partnership_route_id: payload.partnershipRouteId ?? null,
      fee_snapshot_jsonb: payload.feeSnapshotJsonb ?? null,
