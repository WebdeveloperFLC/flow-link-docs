...
function readQualSources(): string {
  const files = [
    "src/lib/application/applicationApi.ts",
    "src/components/application/ApplicationTabContent.tsx",
  ];
  return files.map((f) => readFileSync(resolve(f), "utf8")).join("\n");
}

describe("application non-goals", () => {
  it("application module does not import commission or SFL balance helpers", () => {
