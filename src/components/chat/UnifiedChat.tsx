...
const CONFIG: Record<ChannelType, { label: string; badge?: { text: string; tone: string }; Icon: React.ElementType }> =
  {
    staff_internal: {
      label: "Internal team thread",
      badge: { text: "Hidden from client", tone: "bg-amber-500/15 text-amber-700 dark:text-amber-300" },
      Icon: Lock,
    },
    staff_client: {
      label: "Chat with client",
      badge: { text: "Client visible", tone: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300" },
      Icon: MessageCircle,
    },
...
export function UnifiedChat({
  ...
  useEffect(() => {
    let alive = true;
    listMessages({ channelType, clientId, channelId })
      .then((rows) => {
    ...
    <Card className={cn("flex flex-col h-[420px] overflow-hidden", className)}>
      <div className="px-4 py-3 border-b flex items-center gap-2 bg-muted/30">
        ...
        {cfg.badge && (
          <span
            className={cn(
              "ml-auto text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-semibold",
              cfg.badge.tone,
            )}
          >
            {cfg.badge.text}
      ...
      {enableEnhanced && pinned.length > 0 && (
        <div className="px-3 py-1.5 border-b bg-amber-500/5 text-[11px] text-amber-700 dark:text-amber-300 flex items-center gap-1.5 overflow-x-auto whitespace-nowrap">
          ...
          {pinned.map((p) => (
            <span key={p.id} className="px-1.5 py-0.5 rounded bg-amber-500/10 truncate max-w-[200px]">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        {visibleMessages.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className={cn("group flex flex-col max-w-[85%]", mine ? "ml-auto items-end" : "items-start")}
            >
              <div
                className={cn(
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-1.5">
                ...
                {enableEnhanced && (
                  <span className="opacity-0 group-hover:opacity-100 transition flex items-center gap-1">
                    ...
                    <button className="hover:text-foreground" onClick={() => setReplyTo(m)} title="Reply">
                      <Reply className="size-3" />
                    </button>
                    <button
                      className="hover:text-foreground"
                      onClick={() => togglePin(m.id, !mm?.pinned)}
                      title={mm?.pinned ? "Unpin" : "Pin"}
                    >
                ...
                <div className="flex gap-1 mt-1">
                  {Object.entries(myReactions).map(([emoji, users]) => (
                    <button
                      ...
                      className={cn(
                        "text-[11px] px-1.5 py-0.5 rounded-full border bg-background hover:bg-muted",
      ...
      <div className="border-t p-2 flex gap-2 items-center">
        ...
        />
        <Button onClick={send} disabled={busy || (!input.trim() && !pendingFile)} size="icon">
