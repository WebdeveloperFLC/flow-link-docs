 ...
 */
export function InlinePreviewDialog({ open, onOpenChange, title, url, mime, fileName }: Props) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[95vw] h-[88vh] p-0 overflow-hidden flex flex-col">
        <DialogHeader className="px-4 py-3 border-b">
          <DialogTitle className="text-sm truncate flex items-center justify-between gap-3">
                ...
                <>
                  <Button asChild size="sm" variant="ghost" className="h-7 text-xs">
                    <a href={url} target="_blank" rel="noopener noreferrer">
                      <ExternalLink className="size-3.5 mr-1" /> Open in new tab
                    </a>
                  </Button>
                  <Button asChild size="sm" variant="ghost" className="h-7 text-xs">
