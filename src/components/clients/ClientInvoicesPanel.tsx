...
import { notifyUsers, resolveCounselorNotificationUserIds, resolveAllClientStakeholderUserIds } from "@/lib/appNotifications";
      ...
      <DialogContent className="sm:max-w-4xl max-w-[95vw] max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={requestSave} disabled={saving || proofMissing || overpay || sumPayNow <= 0 || selectedRows.length === 0}>
            {saving ? "Posting…" : willBeAwaitingVerification || !isAccountsUser ? "Submit for verification" : "Post payment"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <AlertDialog open={confirmOpen} onOpenChange={(o) => !saving && setConfirmOpen(o)}>
      ...
      >
        ...
        <AlertDialogFooter className="pt-2">
            ...
            <AlertDialogAction
              disabled={saving}
              onClick={(e) => { e.preventDefault(); void save(!isAccountsUser || willBeAwaitingVerification); }}
            >
              {saving ? "Posting…" : ((!isAccountsUser || willBeAwaitingVerification) ? "Submit for verification" : "Confirm payment received")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
