...
export function ClientInvoicesPanel({
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "client_invoices", filter: `client_id=eq.${clientId}` },
        () => load(),
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "client_invoice_payments", filter: `client_id=eq.${clientId}` },
        () => load(),
      )
      .subscribe();
    return () => {
    ...
    <Card className="p-5">
          ...
          <div className="text-xs text-muted-foreground mt-0.5">
            {loading ? (
              ...
              <>
                ...
                {pending.length > 0 && (
                  <span className="ml-2 text-amber-700">· Awaiting verification {pending.length}</span>
                )}
              </>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="py-6 text-center text-sm text-muted-foreground">
          No invoices yet. Click <b>Create invoice</b> to draft one.
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <Badge
                        variant="outline"
                        className={STATUS_STYLE[totals.displayStatus] ?? "bg-muted text-muted-foreground"}
                      >
                    ...
                    <td
                      className={`px-3 py-2 text-right tabular-nums font-medium ${balance > 0 ? "" : "text-muted-foreground"}`}
                    >
                      {money(balance, r.currency)}
                    </td>
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="ml-1"
                        disabled={remLocked || balance <= 0}
                        title={remLocked ? "An external reminder was already sent for this invoice today." : undefined}
              ...
              return (
                <tr key={p.id} className="border-t align-middle">
                  ...
                  <td className="px-3 py-2">
                    ...
                    <span className="text-muted-foreground">{(p.payment_source || "—").replace(/_/g, " ")}</span>
                  </td>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm">
                <div>
                  This will mark the payment as <b>verified</b> and reduce the invoice outstanding. This action is
                  auditable.
                </div>
                {verifyFor && (
                  <div className="rounded-md border bg-muted/30 p-2 text-xs space-y-1">
                    <div>
                      <b>Invoice:</b> {invoices.find((i) => i.id === verifyFor.invoice_id)?.invoice_number ?? "—"}
                    </div>
                    <div>
                      <b>Amount:</b> {money(Number(verifyFor.amount), verifyFor.currency)}
                    </div>
                    <div>
                      <b>Method:</b> {verifyFor.method?.replace(/_/g, " ")}
                    </div>
                    <div>
                      <b>Reference:</b> {verifyFor.reference || "—"}
                    </div>
                  </div>
                )}
                <div>
...
function CollectPaymentDialog({ invoice, onClose }: { invoice: Invoice; onClose: () => void }) {
    ...
    (async () => {
      ...
      const [{ data: insts }, { data: allocs }, { data: invFull }] = await Promise.all([
        supabase
          ...
          .select(
            "id,installment_number,installment_label,installment_due_date,installment_amount,paid_amount,fee_category",
          )
      ...
      const lineItems: any[] = Array.isArray(invFull?.line_items)
        ? (invFull!.line_items as any[])
      ...
      for (const a of allocList) {
        ...
        if (a.installment_id)
          paidByInstallment.set(a.installment_id, (paidByInstallment.get(a.installment_id) ?? 0) + amt);
        else if (a.line_item_key) paidByLineKey.set(a.line_item_key, (paidByLineKey.get(a.line_item_key) ?? 0) + amt);
        else if (a.service_id)
          paidByServiceFallback.set(a.service_id, (paidByServiceFallback.get(a.service_id) ?? 0) + amt);
      }
      ...
      if (instRows.length > 0) {
        ...
        const firstMeta = firstLine
          ? meta(firstLine)
          : { service_name: "Service", country: null, category: null, service_type: null };
  ...
  const buildLabel = (r: Row) => {
    if (r.kind === "installment")
      return `${r.service_name} — ${r.installment_label ?? `Installment ${r.installment_number ?? ""}`}`.trim();
  ...
  const requestSave = () => {
    ...
    if (overpay) {
      toast.error("A row exceeds its outstanding amount");
      return;
    }
    if (proofMissing) {
      toast.error("Attach a payment proof to continue.");
      return;
    }
    setConfirmNote("");
    setConfirmOpen(true);
  ...
  const save = async (forceAwaiting: boolean) => {
    ...
    try {
      ...
      try {
              ...
              payload: {
                ...
                invoice: {
                  ...
                  outstanding: Math.max(
                    Number(invoice.amount || 0) - (Number(invoice.amount_paid || 0) + totalPayInPayCcy),
                    0,
                  ),
                },
                payment: {
        ...
        console.warn("[payment] notif_dispatch_throw", e); /* never block payment flow on email */
      }
      ...
      try {
        ...
        // When a payment enters the verification queue, alert the accounting
        // team so they can review it promptly.
        if (!verified) {
          const accountingTeam = await resolveAccountingVerifierUserIds({
            context: "payment_awaiting_verification",
            paymentId,
          });
          if (accountingTeam.length) {
            notifyUsers({
              userIds: accountingTeam,
              category: "urgent_review_required",
              severity: "warning",
              title: `Payment needs verification: ${payCcy} ${totalPayInPayCcy.toFixed(2)}`,
              body: `${(cli as any)?.full_name ?? "Client"} • Invoice ${invoice.invoice_number} • via ${method.replace(/_/g, " ")}`,
              link: "/accounting/ar/verification",
              entityType: "invoice_payment",
              entityId: paymentId,
              dedupeKey: `verification:${paymentId}`,
              metadata: { invoice_id: invoice.id, amount: totalPayInPayCcy, currency: payCcy, method },
            });
          }
        }
    ...
    if (pay > 0)
      return (
        <Badge variant="outline" className="bg-amber-500/10 text-amber-700 border-amber-500/20">
          Partial · {money(after, invCcy)} left
        </Badge>
      );
    return (
      <Badge variant="outline" className="bg-muted text-muted-foreground">
        Remaining {money(after, invCcy)}
      </Badge>
    );
  };
    ...
    <>
      <Dialog open onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="sm:max-w-4xl max-w-[95vw] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Collect payment — {invoice.invoice_number}</DialogTitle>
          </DialogHeader>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {loadingRows ? (
                  ...
                  <tr>
                    <td colSpan={9} className="text-center py-4 text-muted-foreground">
                      No service line items on this invoice.
                    </td>
                  </tr>
                ) : (
                  rows.map((r) => {
                    ...
                    return (
                      <tr key={r.key} className="border-t align-middle">
                        ...
                        <td className="px-2 py-1.5">
                          ...
                          {r.payer === "other" && (
                            <Input
                              className="h-6 mt-1 text-[11px]"
                              placeholder="Payer name"
                              value={r.payerName}
                              onChange={(e) => updateRow(r.key, { payerName: e.target.value })}
                            />
                          )}
                        </td>
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            ...
            <div className="col-span-2">
              ...
              <Input
                ...
                placeholder="Txn ID, cheque #, UPI ref…"
              />
            </div>
            <div className="col-span-2">
              <Label>Notes</Label>
          ...
          <div className="rounded-md border bg-muted/30 p-3 text-xs space-y-1 mt-3">
            <div className="flex justify-between">
              ...
              <b>
                {selectedRows.length} · Outstanding {money(selectedOutstanding, invCcy)}
              </b>
            </div>
            <div className="flex justify-between">
              <span>Amount to collect ({invCcy})</span>
              <b>{money(sumPayNow, invCcy)}</b>
            </div>
            {payCcy !== invCcy && (
              <>
                <div className="flex justify-between text-muted-foreground">
                  <span>FX rate</span>
                  <span className="tabular-nums">{fxRate}</span>
                </div>
                <div className="flex justify-between">
                  <span>Charged in {payCcy}</span>
                  <b>{money(amountInPayCcy, payCcy)}</b>
                </div>
              </>
            )}
            <div className="flex justify-between">
            ...
            <div className="flex justify-between pt-1 border-t mt-1">
              ...
              <b>
                {remainingUnpaidServices} {remainingUnpaidServices === 1 ? "service" : "services"}
              </b>
            </div>
            {nextDueRow && (
              <div className="flex justify-between text-muted-foreground">
                <span>Next due</span>
                <span>
                  {buildLabel(nextDueRow)} — {formatDue(nextDueRow.due_date)}
                </span>
              </div>
            )}
            {overpay && <div className="text-destructive">A row's amount exceeds its outstanding balance.</div>}
            {willBeAwaitingVerification && (
              <div className="text-amber-700 pt-1">
                Will be marked <b>awaiting verification</b> — won't reduce outstanding until verified.
              </div>
            )}
          </div>
          ...
          <div className="grid gap-1.5 mt-3">
            ...
            {proofRequired && !adminOverride && (
              <div className="text-xs text-muted-foreground mt-1">Required for all non-cash payment methods.</div>
            )}
            {isAdmin && (
              <label className="mt-2 flex items-start gap-2 rounded-md border border-dashed bg-muted/30 p-2 text-xs cursor-pointer">
                ...
                />
                <span>
                  <b>Admin override</b> — post without proof and mark <b>verified</b> immediately. Skips the
                  verification queue. Action is recorded on the audit trail.
                </span>
              </label>
            )}
          </div>
          ...
          <DialogFooter>
            ...
            >
              {saving
                ? "Posting…"
                : willBeAwaitingVerification || !isAccountsUser
        ...
        <AlertDialogContent className="z-[70] w-[95vw] sm:max-w-[600px] max-h-[88vh] overflow-y-auto p-6 gap-4">
          ...
          <div className="space-y-4 text-sm">
            <div className="rounded-md border bg-muted/30 p-3 text-xs space-y-1">
              ...
              <div>
                <b>Outstanding before:</b> {money(balance, invCcy)}
              </div>
              <div>
                <b>Currency:</b> {invCcy}
                {payCcy !== invCcy ? ` (paid in ${payCcy})` : ""}
              </div>
              <div>
                <b>Method:</b> {method.replace(/_/g, " ")} · <b>Source:</b> {source.replace(/_/g, " ")}
              </div>
              <div>
                <b>Amount:</b> {money(sumPayNow, invCcy)}
                {payCcy !== invCcy ? ` (${money(amountInPayCcy, payCcy)})` : ""}
              </div>
...
function GenerateReceiptDialog({ invoice, onClose }: { invoice: Invoice; onClose: () => void }) {
    ...
    (async () => {
      const [p, f, existing] = await Promise.all([
        supabase
          .from("client_invoice_payments")
          .select(
            "id,amount,currency,method,paid_at,reference,is_refund,payment_status,fx_rate,amount_in_inr,amount_in_cad,amount_in_usd,payment_source,posted_by,verified_by",
          )
          .eq("invoice_id", invoice.id)
          .is("archived_at", null)
          .eq("payment_status", "verified")
  ...
  const save = async () => {
    ...
    const [invRes, firmRes, branchRes, allocRes] = await Promise.all([
      ...
      firmId
        ? supabase
            .from("firm_profile")
            .select("id,firm_name,firm_address,firm_email,firm_phone")
            .eq("id", firmId)
            .maybeSingle()
        : Promise.resolve({ data: null } as any),
      invoice.branch_id
        ? supabase.from("branches").select("id,name,city,country").eq("id", invoice.branch_id).maybeSingle()
        : Promise.resolve({ data: null } as any),
      supabase
        .from("client_invoice_payment_allocations")
    ...
    const allocationsSnapshot = allocRows.map((a, idx) => {
      ...
      const label = inst
        ...
        : category || service_type
          ? `${service_name} — ${category || service_type}`
          : service_name;
      return {
        allocation_id: a.id,
        allocation_order: idx + 1,
    ...
    const userIdsForNames = Array.from(
      new Set(
        [clientRow.assigned_counselor_id, clientRow.owner_id, pay.verified_by, pay.posted_by, u?.user?.id].filter(
          Boolean,
        ),
      ),
    ) as string[];
    ...
    if (userIdsForNames.length) {
      const { data: profs } = await supabase.from("profiles").select("id,full_name").in("id", userIdsForNames);
    ...
    const snapshot = {
      ...
      firm: {
        id: firmRow.id ?? null,
        name: firmRow.firm_name ?? null,
        address: firmRow.firm_address ?? null,
        email: firmRow.firm_email ?? null,
        phone: firmRow.firm_phone ?? null,
        logo_url: firmLogoUrl,
      },
      branch: {
        id: branchRow.id ?? null,
        name: branchRow.name ?? null,
        city: branchRow.city ?? null,
        country: branchRow.country ?? null,
      },
      client: {
    ...
    try {
      ...
      console.warn("[receipt] notif_dispatch_throw", e); /* never block receipt save on email */
    }
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md max-w-[95vw]">
        <DialogHeader>
          <DialogTitle>Generate receipt — {invoice.invoice_number}</DialogTitle>
        </DialogHeader>
        {payments.length === 0 ? (
                ...
                <SelectContent>
                  {payments.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      {new Date(p.paid_at).toLocaleDateString()} · {money(Number(p.amount), p.currency)} · {p.method}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
...
function SendReminderDialog({
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Send reminder — {invoice.invoice_number}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3">
          <div>
            <Label>Channel</Label>
            <Select value={channel} onValueChange={setChannel}>
              ...
              <SelectContent>
                {["whatsapp", "email", "sms", "portal", "internal"].map((c) => (
                  <SelectItem key={c} value={c}>
                    {c}
                  </SelectItem>
                ))}
          ...
          <p className="text-xs text-muted-foreground">
            {channel === "internal"
              ? "Internal note — won't notify the client."
              : "Marked as external — only one external reminder per invoice per day is allowed."}
          </p>
          <div>
            <Label>Notes</Label>
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} />
...
function InvoiceSnapshotDrawer({ invoice, onClose }: { invoice: Invoice; onClose: () => void }) {
    ...
    (async () => {
      const [p, r] = await Promise.all([
        supabase
          .from("client_invoice_payments")
          .select(
            "id,paid_at,method,currency,amount,reference,payment_status,payment_source,fx_rate,is_refund,payment_proof_file_id",
          )
          .eq("invoice_id", invoice.id)
          .is("archived_at", null)
          .order("paid_at", { ascending: false }),
  ...
  const filteredReceipts = !q
    ? receipts
    : receipts.filter((r) =>
        [r.receipt_number, invoice.invoice_number]
          .filter(Boolean)
          .some((v: any) => String(v).toLowerCase().includes(q)),
      );
      ...
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        ...
        <div className="mt-4 space-y-4 text-sm">
          <div className="rounded-md border p-3 grid grid-cols-2 gap-2">
            ...
            <div>
              <div className="text-xs text-muted-foreground">Paid (verified)</div>
              <div className="tabular-nums">{money(verifiedPaid, invoice.currency)}</div>
            </div>
            <div className="col-span-2">
              <div className="text-xs text-muted-foreground">Outstanding</div>
              <div className={`font-semibold tabular-nums ${balance > 0 ? "text-destructive" : "text-emerald-700"}`}>
                {money(balance, invoice.currency)}
              </div>
            </div>
            {awaitingCount > 0 && (
              <div className="col-span-2 text-[11px] text-amber-700">
                {awaitingCount} payment{awaitingCount === 1 ? "" : "s"} awaiting verification — not counted in
                paid/outstanding.
              </div>
            )}
          </div>
          ...
          <div>
            <Input
              placeholder="Search receipt #, ref, method, status…"
              value={search}
                          ...
                          <td className="px-2 py-1 text-right whitespace-nowrap">
                            {p.payment_proof_file_id && (
                              <Button
                                ...
                                variant="ghost"
                                title="View proof"
                                onClick={() => openPaymentProof(p.payment_proof_file_id)}
                              >
                                <Eye className="size-3.5" />
                              </Button>
                            )}
                            {rcpt?.receipt_snapshot_jsonb && (
                              <Button
                                ...
                                onClick={() => printReceiptSnapshot(rcpt.receipt_snapshot_jsonb)}
                              >
                        ...
                        <tr key={r.id} className="border-t">
                          ...
                          <td className="px-2 py-1 text-right whitespace-nowrap">
                            <Button
                              ...
                              variant="ghost"
                              disabled={!snap}
                              title={snap ? "Print / Download PDF" : "Snapshot unavailable"}
                              onClick={() => snap && printReceiptSnapshot(snap)}
                            >
                              <Download className="size-3.5" />
                            </Button>
                          </td>
          ...
          <div className="text-[11px] text-muted-foreground italic">
            Read-only snapshot. Use the row actions to modify.
          </div>
        </div>
      </SheetContent>
    </Sheet>
  ...
  const filtered = !q.trim()
    ...
    : rows.filter((r) =>
        [r.receipt_number, r.invoice_number]
          .filter(Boolean)
          .some((v: any) => String(v).toLowerCase().includes(q.trim().toLowerCase())),
      );
  return (
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>All receipts</SheetTitle>
        </SheetHeader>
        <div className="mt-4 space-y-3">
          ...
          {loading ? (
            ...
            <div className="py-6 text-center text-sm text-muted-foreground">
              {rows.length === 0 ? "No receipts generated yet." : "No receipts match your search."}
            </div>
          ) : (
            <div className="rounded-md border overflow-x-auto">
              <table className="w-full text-xs">
                    ...
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="px-2 py-1 text-right">
                        <Button
                          size="sm"
                          variant="ghost"
                          disabled={!r.receipt_snapshot_jsonb}
                          title={r.receipt_snapshot_jsonb ? "Print / Download PDF" : "Snapshot unavailable"}
                          onClick={() => r.receipt_snapshot_jsonb && printReceiptSnapshot(r.receipt_snapshot_jsonb)}
                        >
                          <Download className="size-3.5" />
                        </Button>
                      </td>
          ...
          <div className="text-[11px] text-muted-foreground italic">
            Generated receipts are stored permanently. Voided receipts remain in the list for audit.
          </div>
        </div>
      </SheetContent>
    </Sheet>
