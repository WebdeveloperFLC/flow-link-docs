...
export function summarizeProfileSection(vm: ProfileViewModel, section: ProfileSectionId): ProfileSectionSummary {
  switch (section) {
    ...
    case "tests": {
      const active = vm.tests.english.find((e) => e.test_type === vm.tests.active_english_test);
      const lines: string[] = [];
      if (active) {
        const variant = active.ielts_variant ? ` (${active.ielts_variant})` : "";
        lines.push(
          `English: ${active.test_type}${variant} — ${active.status ?? "—"}${active.overall ? `, ${active.overall}` : ""}`,
        );
      }
      for (const a of vm.tests.aptitude.slice(0, 2)) {
        lines.push(`${a.test_type}: ${a.overall ?? a.status ?? "—"}`);
      }
      for (const l of vm.tests.language.slice(0, 1)) {
        lines.push(`${l.language}: ${l.exam_type ?? "—"} ${l.overall_score ?? ""}`.trim());
      }
...
export function summarizeProfileFor360(vm: ProfileViewModel): Client360ProfileSummary {
  ...
  const active = vm.tests.english.find((e) => e.test_type === vm.tests.active_english_test);
  if (active?.overall) highlights.push(`${active.test_type} ${active.overall}`);
