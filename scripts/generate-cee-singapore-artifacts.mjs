#!/usr/bin/env node
/**
 * FAQ + checklist + parity migration for Poland, Hungary, Latvia, Singapore, Finland family.
 * Run: node scripts/generate-cee-singapore-artifacts.mjs
 */
import fs from "fs";
import path from "path";
import { spawnSync } from "child_process";
import { LIBRARY_IDS } from "./lib/service-library-ids.mjs";
import { CEE_SINGAPORE_FILES } from "./lib/cee-singapore-visa-registry.mjs";
...
function expandFaqs(meta) {
  ...
  const generic = [
    ...
    { q: "How long does processing take?", a: "See KPI processing time — verify current embassy/ICA timelines before quoting." },
    { q: "What are government fees?", a: "Confirm latest fees on the official portal in the Resources section before quoting." },
    { q: "Can the client apply from India?", a: "Yes — standard route for Indian nationals via embassy, VFS, or online portal as applicable." },
    { q: "What if the client was refused before?", a: "Disclose all refusals and overstays; address gaps before reapply." },
  ];
  const out = [];
  const seen = new Set();
...
const faqParts = ["-- CEE + Singapore visa FAQ seeds", "-- Regenerate: node scripts/generate-cee-singapore-artifacts.mjs", ""];
const clParts = ["-- CEE + Singapore submission checklist seeds", ""];
const checklistLinks = [];
const logoSrc = `data:image/png;base64,${fs.readFileSync(LOGO).toString("base64")}`;

/** Safe file_name for SQL — no apostrophes (Lovable SQL editor chokes on them). */
function safeChecklistFileName(displayName) {
  return `${displayName.replace(/'/g, "")} — Document Checklist.html`;
}

for (const file of CEE_SINGAPORE_FILES) {
  ...
  const displayName = spec.displayName ?? slugName;
  checklistLinks.push({
    id,
    slug: slugName,
    fileName: safeChecklistFileName(displayName),
    bytes: Buffer.byteLength(html),
    items: countItems(spec),
  });
  console.log(`HTML ${slugName}.html`);
}

const linkRows = checklistLinks
  .map(
    (l) =>
      `  ('${l.id}'::uuid, '${l.fileName.replace(/'/g, "")}', '/specimens/checklists/${l.slug}.html', 'text/html', ${l.bytes}, 1, true, 'Future Link branded checklist — fields auto-fill when linked to client')`,
  )
  .join(",\n");

// Checklists only — FAQs/quizzes ship via visa-metadata-seed/batches/*.sql (avoids 90KB+ JSON in one paste).
const paritySql = [
...
fs.writeFileSync(path.join(process.cwd(), "supabase/migrations/20260615101000_seed_cee_singapore_parity.sql"), paritySql);

// Optional per-service FAQ patches (run individually if needed; not bundled into Lovable migration).
const faqBundle = [...faqParts].join("\n");
fs.writeFileSync(
  path.join(process.cwd(), "supabase/migrations/faq-seed/cee_singapore_faq_bundle.sql"),
  faqBundle,
);

spawnSync(process.execPath, ["scripts/generate-visa-metadata-sql-split.mjs"], { cwd: process.cwd(), stdio: "inherit" });
spawnSync(process.execPath, ["scripts/generate-eligibility-questions-sql-split.mjs"], { cwd: process.cwd(), stdio: "inherit" });

console.log("Done CEE + Singapore artifacts.");
