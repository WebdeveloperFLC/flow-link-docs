#!/usr/bin/env node
/**
 * FAQ + checklist seeds + parity migration for Cyprus & Lithuania visa services.
 * Run: node scripts/generate-cyprus-lithuania-artifacts.mjs
 */
import fs from "fs";
...
const FILES = [
  "cyprus-student-visa.json",
  "cyprus-visitor-visa.json",
  "lithuania-student-visa.json",
  "lithuania-visitor-visa.json",
];
...
const faqParts = ["-- Cyprus + Lithuania FAQ seeds", "-- Regenerate: node scripts/generate-cyprus-lithuania-artifacts.mjs", ""];
const clParts = ["-- Cyprus + Lithuania submission checklist seeds", ""];
...
for (const file of FILES) {
  ...
  const outPath = path.join(OUT_DIR, `${slugName}.html`);
...
fs.writeFileSync(
  path.join(process.cwd(), "supabase/migrations/20260612101000_seed_cyprus_lithuania_parity.sql"),
  paritySql,
);

spawnSync(process.execPath, ["scripts/generate-all-service-checklist-specimens.mjs"], {
  cwd: process.cwd(),
  stdio: "inherit",
});

spawnSync(process.execPath, ["scripts/generate-visa-metadata-sql-split.mjs"], {
  cwd: process.cwd(),
  stdio: "inherit",
});

console.log("Done Cyprus/Lithuania artifacts.");
